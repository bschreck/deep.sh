...
Built-in Aliases
********************
This page describes the deepsh built-in commands and aliases.
...
.. command-help:: deepsh.dirstack.pushd
...
.. command-help:: deepsh.dirstack.popd
...
.. command-help:: deepsh.dirstack.dirs
...
leave deepsh in a safe manner. Typing ``Ctrl-d`` is the same as typing ``EOF`` and
pressing enter.
...
``source-foreign``
====================
Like the ``source`` command but for files in foreign (non-deepsh) languages.
It will pick up the environment and any aliases.

.. command-help:: deepsh.aliases.source_foreign


``history``
====================
Tools for dealing with deepsh history. See `the history tutorial <tutorial_hist.html>`_
for more information all the history command and all of its sub-commands.

.. command-help:: deepsh.history.main.history_main
...
.. command-help:: deepsh.tracer.tracermain


``xpip``
=================
Runs the ``pip`` package manager for deepsh itself. Useful for installations where deepsh is in an
isolated environment (eg conda, homebrew).

In general, use ``xpip`` if you're configuring or adding features to deepsh, and use ``pip`` if
...
``sudo`` on Windows
====================
On Windows, if no executables named ``sudo`` are found, Deepsh adds a ``sudo`` alias
