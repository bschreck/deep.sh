...
def _which_create_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="all",
        help="Show all matches in globals, deepsh.aliases, $PATH",
    )
    parser.add_argument(
        "-s",
        "--skip-alias",
        action="store_true",
        help="Do not search in deepsh.aliases",
        dest="skip",
    )
    parser.add_argument(
        "-V",
        ...
        help="Display the version of the python which module " "used by deepsh",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def print_alias(arg, stdout, verbose=False):
    ...
    if not verbose:
        ...
        elif isinstance(alias, deepsh.aliases.ExecAlias):
        ...
        if callable(alias) and not isinstance(alias, deepsh.aliases.ExecAlias):
            XSH.superhelp(alias)


def which(args, stdin=None, stdout=None, stderr=None, spec=None):
    """
    Checks if each arguments is a deepsh aliases, then if it's an executable,
    then finally return an error code equal to the number of misses.
    If '-a' flag is passed, run both to return both `deepsh` match and
    `which` match.
    """
    ...
    if len(failures) == 0:
        ...
        if not pargs.skip:
            print(" or deepsh.builtins.aliases", file=stderr, end="")
...
class AWitchAWitch(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        webbrowser.open("https://github.com/deepsh/deepsh/commit/f49b400")
        parser.exit()
