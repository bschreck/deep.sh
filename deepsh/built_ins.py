...
def subproc_captured_stdout(*cmds, envs=None):
    ...
    that was produced as a str or list based on ``$DEEPSH_SUBPROC_OUTPUT_FORMAT``.
    """

    import deepsh.procs.specs

    return deepsh.procs.specs.run_subproc(cmds, captured="stdout", envs=envs)
...
def subproc_captured_inject(*cmds, envs=None):
    ...
    The string is split using deepsh's lexer, rather than Python's str.split()
    or shlex.split().
    """
    import deepsh.procs.specs

    o = deepsh.procs.specs.run_subproc(cmds, captured="object", envs=envs)
    o.end()
    toks = []
    for line in o:
...
def subproc_captured_object(*cmds, envs=None):
    ...
    return deepsh.procs.specs.run_subproc(cmds, captured="object", envs=envs)
...
def subproc_captured_hiddenobject(*cmds, envs=None):
    ...
    return deepsh.procs.specs.run_subproc(cmds, captured="hiddenobject", envs=envs)
...
def subproc_uncaptured(*cmds, envs=None):
    ...
    return deepsh.procs.specs.run_subproc(cmds, captured=False, envs=envs)
...
def eval_fstring_field(field):
    """Evaluates the argument in Deepsh context."""
    res = XSH.execer.eval(
...
def enter_macro(obj, raw_block, glbs, locs):
    ...
    kind = getattr(obj, "__deepsh_block__", str)
...
@contextlib.contextmanager
def deepsh_builtins(execer=None):
    """A context manager for using the deepsh builtins only in a limited
...
class DeepshSession:
    """All components defining a deepsh session."""
    ...
    def __init__(self):
        ...
        ----------
        exit: int or None
            Session attribute. In case of integer value it signals deepsh to exit
    ...
    def completers(self):
        ...
        if self._completers is None:
            from deepsh.completers.init import default_completers
    ...
    def load(self, execer=None, ctx=None, inherit_env=True, **kwargs):
        ...
        if "env" in kwargs:
            ...
            self.env = Env({"DEEPSH_ENV_INHERITED": False})
    ...
    def link_builtins(self):
        ...
        for refname in self._initial_builtin_names:
            objname = f"__deepsh__.builtins.{refname}"
        ...
        # relies on __deepsh__.env in default aliases
...
def get_default_builtins(execer=None):
    ...
    return types.SimpleNamespace(
        DeepshError=DeepshError,
        DeepshCalledProcessError=DeepshCalledProcessError,
