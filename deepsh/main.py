...
from deepsh.codecache import run_code_with_cache, run_script_with_cache
from deepsh.environ import get_home_deepshrc_path, make_args_env, deepshrc_context
from deepsh.events import events
from deepsh.execer import Execer
from deepsh.imphooks import install_import_hooks
from deepsh.lib.lazyasd import lazyobject
from deepsh.lib.lazyimps import pyghooks, pygments
from deepsh.lib.pretty import pretty
from deepsh.platform import HAS_PYGMENTS, ON_WINDOWS
from deepsh.procs.jobs import ignore_sigtstp
from deepsh.shell import Shell
from deepsh.timings import setup_timings
...
@lazyobject
def parser():
    p = argparse.ArgumentParser(description="deepsh", add_help=False)
    p.add_argument(
        "-h",
        "--help",
        dest="help",
    ...
    p.add_argument(
        "--rc",
        help="The deepshrc files to load, these may be either deepsh "
        "files or directories containing deepsh files",
        dest="rc",
        nargs="+",
        type=path_argument,
        default=None,
    ...
    p.add_argument(
        "--no-rc",
        help="Do not load any deepsh RC files. Argument --rc will "
...
def _pprint_displayhook(value):
    ...
    if HAS_PYGMENTS and env.get("COLOR_RESULTS"):
        tokens = list(pygments.lex(printed_val, lexer=pyghooks.DeepshLexer()))
...
def _get_rc_files(shell_kwargs: dict, args, env):
    ...
    if rc_cli:
        ...
        # file, and nothing else (ignore both DEEPSHRC and DEEPSHRC_DIR)
        rc = tuple(r for r in rc_cli if os.path.isfile(r))
        rcd = tuple(r for r in rc_cli if os.path.isdir(r))
        return rc, rcd

    # otherwise, get the RC files from DEEPSHRC, and RC dirs from DEEPSHRC_DIR
    rc = env.get("DEEPSHRC")
    rcd = env.get("DEEPSHRC_DIR")

    if not env.get("DEEPSH_INTERACTIVE", False):
        """
        Home based ``~/.deepshrc`` file has special meaning and history. The ecosystem around shells treats this kind of files
        ...
        """
        home_deepshrc = get_home_deepshrc_path()
        rc = tuple(c for c in rc if c != home_deepshrc)
...
def start_services(shell_kwargs, args, pre_env=None):
    ...
    debug = to_bool_or_int(os.getenv("DEEPSH_DEBUG", "0"))
    ...
    events.on_timingprobe.fire(name="pre_deepsh_session_load")
    XSH.load(ctx=ctx, execer=execer, inherit_env=shell_kwargs.get("inherit_env", True))
    events.on_timingprobe.fire(name="post_deepsh_session_load")
...
def premain(argv=None):
    """Setup for main deepsh entry point. Returns parsed arguments."""
    if argv is None:
        argv = sys.argv[1:]
    setup_timings(argv)
    setproctitle = get_setproctitle()
    if setproctitle is not None:
        setproctitle(" ".join(["deepsh"] + argv))
    ...
    if args.command is not None:
        args.mode = DeepshMode.single_command
        shell_kwargs["shell_type"] = "none"
        deepsh_mode = "single_command"
    elif args.file is not None:
        args.mode = DeepshMode.script_from_file
        shell_kwargs["shell_type"] = "none"
        deepsh_mode = "script_from_file"
    elif not sys.stdin.isatty() and not args.force_interactive:
        args.mode = DeepshMode.script_from_stdin
        shell_kwargs["shell_type"] = "none"
        deepsh_mode = "script_from_stdin"
    else:
        args.mode = DeepshMode.interactive
        shell_kwargs["completer"] = True
        shell_kwargs["login"] = True
        deepsh_mode = "interactive"

    pre_env = {
        "DEEPSH_LOGIN": shell_kwargs["login"],
        "DEEPSH_INTERACTIVE": args.force_interactive
        or (args.mode == DeepshMode.interactive),
        "DEEPSH_MODE": deepsh_mode,
    }
    pre_env["COLOR_RESULTS"] = os.getenv("COLOR_RESULTS", pre_env["DEEPSH_INTERACTIVE"])
...
def _failback_to_other_shells(args, err):
    ...
    if hasattr(args, "mode") and args.mode != DeepshMode.interactive:
        raise err

    foreign_shell = None

    # look first in users login shell $SHELL.
    # use real os.environ, in case Deepsh hasn't initialized yet
    ...
    if foreign_shell:
        traceback.print_exc()
        print("Deepsh encountered an issue during launch.", file=sys.stderr)
        print("Please report to https://github.com/deepsh/deepsh/issues", file=sys.stderr)
...
def main(argv=None):
    ...
    try:
        args = premain(argv)
        sys.exit(main_deepsh(args))
    except Exception as err:
        _failback_to_other_shells(args, err)


def main_deepsh(args):
    """Main entry point for deepsh cli."""
    if not ON_WINDOWS:
    ...
    if shell and not env["DEEPSH_INTERACTIVE"]:
    ...
    # if this does not change, no exceptions were thrown. Otherwise, print a traceback that does not expose deepsh internals
    exc_info = None, None, None

    try:
        if args.mode == DeepshMode.interactive:
            # enter the shell

            # Setted again here because it is possible to call main_deepsh() without calling premain(), namely in the tests.
            env["DEEPSH_INTERACTIVE"] = True

            ignore_sigtstp()
            if (
                env["DEEPSH_INTERACTIVE"]
                and not env["DEEPSH_SUPPRESS_WELCOME"]
                and sys.stdin.isatty()  # In case the interactive mode is forced but no tty (input from pipe).
                and not any(os.path.isfile(i) for i in env["DEEPSHRC"])
                and not any(os.path.isdir(i) for i in env["DEEPSHRC_DIR"])
            ):
        ...
        elif args.mode == DeepshMode.single_command:
            ...
            if os.path.isfile(path):
                ...
                print(f"deepsh: {args.file}: No such file or directory.")
                exit_code = 1
        elif args.mode == DeepshMode.script_from_stdin:
        ...
        exit_code = 1
        debug_level = env.get("DEEPSH_DEBUG", 0)
        if debug_level == 0:
...
def postmain(args=None):
    """Teardown for main deepsh entry point, accepts parsed arguments."""
    XSH.unload()
    XSH.shell = None
...
def setup(
    ...
    packages ``__init__.py`` will allow deepsh to be fully used in the
    package in headless or headed mode. This function is primarily indended to
    make starting up deepsh for 3rd party packages easier.
    ...
    ctx : dict-like or None, optional
        The deepsh context to start with. If None, an empty dictionary
        is provided.
    ...
    # setup deepsh ctx and execer
    if not hasattr(builtins, "__deepsh__"):
