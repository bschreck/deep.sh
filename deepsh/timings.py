"""Timing related functionality for the deepsh shell.
...
def timeit_alias(args, stdin=None):
    ...
    innercode = XSH.builtins.compilex(
        innerstr, filename="<deepsh-timeit>", mode="exec", glbs=ctx
    )
    tc = clock() - t0
    # get inner func
    ns = {}
...
def setup_timings(argv):
    ...
    if "--timings" in argv:
        ...
        def timing_on_pre_prompt(**kw):
            ...
            # Print result table
            print(" Debug level: {}".format(os.getenv("DEEPSH_DEBUG", "Off")))
            print(sepline)
