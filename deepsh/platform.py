...
from deepsh.lib.lazyasd import LazyBool, lazybool, lazyobject

# do not import any deepsh-modules here to avoid circular dependencies
...
"""Minimum version of prompt-toolkit supported by Deepsh"""
...
def windows_expanduser(path):
    """A Windows-specific expanduser() function for deepsh. This is needed
...
def windows_bash_command():
    ...
    if bash_on_path:
        try:
            ...
            # which can't be used by deepsh foreign-shell/completer
...
def bash_command():
    ...
    if (bc := os.getenv("DEEPSH_BASH_PATH_OVERRIDE", None)) is not None:
