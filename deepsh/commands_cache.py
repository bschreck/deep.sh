...
from deepsh.platform import ON_POSIX, ON_WINDOWS, pathbasename
...
class CommandsCache(cabc.Mapping):
    ...
    Note! There is ``deepsh.procs.executables`` module with resolving executables.
    ...
    def cache_file(self):
        ...
        if self._cache_file is None:
            if "DEEPSH_CACHE_DIR" in env and env.get("COMMANDS_CACHE_SAVE_INTERMEDIATE"):
                self._cache_file = (
                    Path(env["DEEPSH_CACHE_DIR"]).joinpath(self.CACHE_FILE).resolve()
                )
            else:
    ...
    def locate_binary(self, name, ignore_alias=False):
        ...
        NOT RECOMMENDED. Take a look into `deepsh.procs.executables.locate_executable`
...
def default_threadable_predictors():
    ...
    predictors = {
        ...
        "co": predict_help_ver,
        "con.sh": predict_shell,
        "deepsh": predict_shell,
        "yes": predict_false,
        "zip": predict_true,
