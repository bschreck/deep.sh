"""Key bindings for prompt_toolkit deepsh shell."""
...
from deepsh.aliases import deepsh_exit
...
def carriage_return(b, cli, *, autoindent=True):
    ...
    deepsh parser for execution or should insert a newline for continued input.
...
def can_compile(src):
    """Returns whether the code can be compiled, i.e. it is valid deepsh."""
...
def autopair_condition():
    """Check if DEEPSH_AUTOPAIR is set"""
    return XSH.env.get("DEEPSH_AUTOPAIR", False)
...
def load_deepsh_bindings(ptk_bindings: KeyBindingsBase) -> KeyBindingsBase:
    """
    ...
    if XSH.env["DEEPSH_CTRL_BKSP_DELETION"]:
        ...
        # See https://github.com/deepsh/deepsh/issues/4407
