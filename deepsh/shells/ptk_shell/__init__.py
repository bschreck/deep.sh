"""The prompt_toolkit based deepsh shell."""
...
from deepsh.lib.lazyimps import pyghooks, pygments, winutils
from deepsh.platform import HAS_PYGMENTS, ON_POSIX, ON_WINDOWS
...
from deepsh.shells.ptk_shell.formatter import PTKPromptFormatter
from deepsh.shells.ptk_shell.history import PromptToolkitHistory, _cust_history_matches
from deepsh.shells.ptk_shell.key_bindings import load_deepsh_bindings
from deepsh.style_tools import DEFAULT_STYLE_DICT, _TokenType, partial_color_tokenize
from deepsh.tools import carriage_return, print_exception, print_warning
...
def disable_copy_on_deletion():
    ...
    def handle_binding(name):
        ...
        if getattr(binding, "deepsh_disabled_clipboard", False):
            # binding's clipboard has already been disabled
            return

        binding.deepsh_disabled_clipboard = True
...
class PromptToolkitShell(BaseShell):
    """The deepsh shell for prompt_toolkit v2 and later."""
    ...
    def __init__(self, **kwargs):
        if not XSH.env.get("DEEPSH_DEBUG", False):
        ...
        if not XSH.env.get("DEEPSH_COPY_ON_DELETE", False):
            disable_copy_on_deletion()
        if HAVE_SYS_CLIPBOARD and (XSH.env.get("DEEPSH_USE_SYSTEM_CLIPBOARD", True)):
        ...
        self.key_bindings = load_deepsh_bindings(ptk_bindings)
    ...
    def get_lazy_ptk_kwargs(self):
        ...
        if HAS_PYGMENTS:
            ...
            yield "lexer", PygmentsLexer(pyghooks.DeepshLexer)
    ...
    def get_prompt_style(self):
        ...
        if (
            ...
            print_warning(
                "$PTK_STYLE_OVERRIDES is deprecated, use $DEEPSH_STYLE_OVERRIDES instead!"
            )
            self._overrides_deprecation_warning_shown = True
        style_overrides_env.update(env.get("DEEPSH_STYLE_OVERRIDES", {}))

        if HAS_PYGMENTS:
            style = _style_from_pygments_cls(pyghooks.deepsh_style_proxy(self.styler))
    ...
    def singleline(
        ...
        if HAS_PYGMENTS:
            self.styler.style_name = env.get("DEEPSH_COLOR_STYLE")
        ...
        if env.get("DEEPSH_HISTORY_MATCH_ANYWHERE"):
        ...
        if menu_rows:
            # https://github.com/deepsh/deepsh/pull/4477#pullrequestreview-767982976
            menu_rows += 1
    ...
    def format_color(self, string, hide=False, force_string=False, **kwargs):
        ...
        if force_string and HAS_PYGMENTS:
            env = XSH.env
            style_overrides_env = env.get("DEEPSH_STYLE_OVERRIDES", {})
            self.styler.style_name = env.get("DEEPSH_COLOR_STYLE")
            self.styler.override(style_overrides_env)
            proxy_style = pyghooks.deepsh_style_proxy(self.styler)
            formatter = pyghooks.DeepshTerminal256Formatter(style=proxy_style)
    ...
    def print_color(self, string, end="\n", **kwargs):
        ...
        if HAS_PYGMENTS:
            ...
            proxy_style = _style_from_pygments_cls(
                pyghooks.deepsh_style_proxy(self.styler)
            )
        else:
    ...
    def color_style_names(self):
        ...
        if not HAS_PYGMENTS:
            return ["For other deepsh styles, please install pygments"]
