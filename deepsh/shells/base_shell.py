"""The base class for deepsh shell"""
...
from deepsh.prompt.base import PromptFormatter, multiline_prompt
...
class _TeeStdBuf(io.RawIOBase):
    ...
    def __init__(
        ...
        encoding : str or None, optional
            ...
            rather than a binary one. Defaults to $DEEPSH_ENCODING if None.
        errors : str or None, optional
            ...
            $DEEPSH_ENCODING_ERRORS if None.
        ...
        self.membuf = membuf
        env = XSH.env
        self.encoding = env.get("DEEPSH_ENCODING") if encoding is None else encoding
        self.errors = env.get("DEEPSH_ENCODING_ERRORS") if errors is None else errors
        self.prestd = prestd
...
class Tee:
    ...
    def __init__(
        ...
        env = XSH.env
        prestderr = format_std_prepost(env.get("DEEPSH_STDERR_PREFIX"))
        poststderr = format_std_prepost(env.get("DEEPSH_STDERR_POSTFIX"))
        self.stderr = _TeeStd(
...
class BaseShell:
    ...
    def styler(self):
        if self._styler is DefaultNotGiven:
            if HAS_PYGMENTS:
                from deepsh.pyghooks import DeepshStyle

                env = XSH.env
                self._styler = DeepshStyle(env.get("DEEPSH_COLOR_STYLE"))
            else:
                self._styler = None
        return self._styler
    ...
    def default(self, line, raw_line=None):
        ...
        ts1 = None
        enc = env.get("DEEPSH_ENCODING")
        err = env.get("DEEPSH_ENCODING_ERRORS")
        ...
        try:
            ...
            if (
                tee_out
                and env.get("DEEPSH_APPEND_NEWLINE")
    ...
    def _fix_cwd(self):
        ...
        if cwd is None:
            ...
            if pwd is None:
                ...
                msg += "deepsh: working directory does not exist: " + pwd
                msg += "{RESET}"
    ...
    def settitle(self):
        ...
        if ON_WINDOWS and "ANSICON" not in env:
            ...
            with open(1, "wb", closefd=False) as f:
                # prevent deepsh from answering interactive questions
    ...
    def print_color(self, string, hide=False, **kwargs):
        ...
        if isinstance(string, str):
            ...
            style_proxy = pyghooks.deepsh_style_proxy(self.styler)
            formatter = pyghooks.DeepshTerminal256Formatter(style=style_proxy)
