"""The readline based deepsh shell.
...
import deepsh.completers.tools as xct
...
from deepsh.prompt.base import multiline_prompt
...
def setup_readline():
    ...
    if readline is None:
        print(
            ...
            `deepsh` interactively.
            See https://github.com/deepsh/deepsh/issues/1170"""
        )
        return
    ...
    if os.path.isfile(inputrc_name):
        try:
            ...
            print_exception("deepsh: could not load readline default init file.")
...
def rl_variable_value(variable):
    ...
    env = XSH.env
    enc, errors = env.get("DEEPSH_ENCODING"), env.get("DEEPSH_ENCODING_ERRORS")
...
class ReadlineShell(BaseShell, cmd.Cmd):
    ...
    def prompt(self):
        ...
        p = ansi_partial_color_format(p, style=env.get("DEEPSH_COLOR_STYLE"), hide=hide)
