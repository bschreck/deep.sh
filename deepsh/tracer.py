...
import deepsh.prompt.cwd as prompt
from deepsh.cli_utils import Annotated, Arg, ArgParserAlias
from deepsh.lib.inspectors import find_file
from deepsh.lib.lazyasd import LazyObject
from deepsh.lib.lazyimps import pyghooks, pygments
from deepsh.platform import HAS_PYGMENTS
from deepsh.tools import DefaultNotGiven, normabspath, print_color, to_bool
...
class TracerType:
    """Represents a deepsh tracer object, which keeps track of all tracing
    ...
    def __init__(self):
        ...
        self.files = set()
        self.usecolor = True
        self.lexer = pyghooks.DeepshLexer()
...
def tracer_format_line(fname, lineno, line, color=True, lexer=None, formatter=None):
    ...
    lexer = lexer or pyghooks.DeepshLexer()
...
def _find_caller(args):
    ...
    for _, fname, lineno, _, lines, _ in inspect.getouterframes(curr, context=1)[3:]:
        ...
        msg = (
            "deepsh: warning: __file__ name could not be found. You may be "
...
class TracerAlias(ArgParserAlias):
    """Tool for tracing deepsh code as it runs."""
