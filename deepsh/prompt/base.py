...
class PromptFormatter:
    ...
    def __call__(self, template=DEFAULT_PROMPT, fields=None, **kwargs) -> str:
        """Formats a deepsh prompt template string."""
...
def _failover_template_format(template):
    if callable(template):
        try:
            ...
            # in user's deepshrc should not crash deepsh
            return template()
        except Exception:
            xt.print_exception()
            return "$ "
...
class PromptFields(tp.MutableMapping[str, "FieldType"]):
    ...
    def __init__(self, xsh: "DeepshSession", init=True):
    ...
    def load_initial(self):
        from deepsh.prompt import gitstatus
        from deepsh.prompt.cwd import (
            _collapsed_pwd,
            _dynamically_collapsed_pwd,
            _replace_home_cwd,
        )
        from deepsh.prompt.env import env_name, vte_new_tab_cwd
        from deepsh.prompt.job import CurrentJobField
        from deepsh.prompt.times import _localtime
        from deepsh.prompt.vc import branch_bg_color, branch_color, current_branch
