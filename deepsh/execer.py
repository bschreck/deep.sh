"""Implements the deepsh executer."""
...
from deepsh.parsers.ast import CtxAwareTransformer
...
class Execer:
    """Executes deepsh code in a context."""

    def __init__(
        self,
        filename="<deepsh-code>",
        debug_level=0,
        parser_args=None,
        scriptcache=True,
        cacheall=False,
        ...
        cacheall : bool, optional
            Whether or not to cache all deepsh code, and not just files. If this
    ...
    def parse(self, input, ctx, mode="exec", filename=None, transform=True):
        """Parses deepsh code in a context-aware fashion. For context-free
    ...
    def compile(
        ...
        """Compiles deepsh code into a Python code object, which may then
        be execed or evaled.
        """
        if filename is None:
            filename = self.filename
    ...
    def eval(
        ...
        """Evaluates (and returns) deepsh code."""
        if glbs is None:
            glbs = {}
