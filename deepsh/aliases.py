...
from deepsh.environ import locate_binary, make_args_env
from deepsh.foreign_shells import foreign_shell_data
from deepsh.lib.lazyasd import lazyobject
from deepsh.parsers.ast import isexpression
...
from deepsh.procs.executables import locate_file
from deepsh.procs.jobs import bg, clean_jobs, disown, fg, jobs
from deepsh.procs.specs import DecoratorAlias, SpecAttrDecoratorAlias
from deepsh.timings import timeit_alias
...
class FuncAlias:
    """Provides a callable alias for deepsh commands."""

    attributes_show = ["__deepsh_threadable__", "__deepsh_capturable__", "return_what"]
...
class ExecAlias:
    """Provides an exec alias for deepsh source code."""
...
class PartialEvalAlias0(PartialEvalAliasBase):
    def __call__(
        ...
        if args:
            ...
            raise DeepshError(msg.format(f=self.f, args=args, acc_args=self.acc_args))
        return self.f()
...
def partial_eval_alias(f, acc_args=()):
    ...
    if numargs < 8:
        ...
        raise DeepshError(e.format(", ".join(ALIAS_KWARG_NAMES), numargs))
...
def deepsh_exit(args, stdin=None):
...
def deepsh_reset(args, stdin=None):
    """Clears __deepsh__.ctx"""
    XSH.ctx.clear()
...
def source_foreign_fn(
    ...
    overwrite_aliases : --overwrite-aliases
        flag for whether or not sourced aliases should replace the current deepsh aliases.
    ...
    if prevcmd:
        ...
        if not sourcer:
            return (None, "deepsh: error: `sourcer` command is not mentioned.\n", 1)
    ...
    if fsenv is None:
        if dryrun:
            ...
            msg = f"deepsh: error: Source failed: {prevcmd!r}\n"
            msg += "deepsh: error: Possible reasons: File not found or syntax error\n"
            return (None, msg, 1)
    # apply results
    denv = env.detype()
    ...
    for k, v in fsaliases.items():
        if k in baliases and v == baliases[k]:
            ...
            msg = (
                ...
                "since it shares a name with an existing deepsh alias. "
...
def source_alias(args, stdin=None):
    ...
    """
    env = XSH.env
    encoding = env.get("DEEPSH_ENCODING")
    errors = env.get("DEEPSH_ENCODING_ERRORS")
    for i, fname in enumerate(args):
        fpath = fname
        if not os.path.isfile(fpath):
            fpath = locate_file(fname)
            if fpath is None:
                if env.get("DEEPSH_DEBUG"):
        ...
        if fext and fext != ".xsh" and fext != ".py":
            raise RuntimeError(
                "attempting to source non-deepsh file! If you are "
        ...
        with (
            env.swap(DEEPSH_MODE="source", **make_args_env(args[i + 1 :])),
            ...
            try:
                ...
                print_color(
                    "{RED}You may be attempting to source non-deepsh file! "
...
def source_cmd_fn(
    ...
    if not os.path.isfile(args[0]):
        return (None, f"deepsh: error: File not found: {args[0]}\n", 1)
    prevcmd = "call "
...
def xexec_fn(
    ...
    replace the deepsh process with the specified program.
    ...
    try:
        ...
        return (
            None,
            f"deepsh: exec: file not found: {e.args[1]}: {command[0]}" "\n",
            1,
        )
...
def trace(args, stdin=None, stdout=None, stderr=None, spec=None):
    """Runs the deepsh tracer utility."""
    from deepsh.tracer import tracermain  # lazy import
...
def showcmd(args, stdin=None):
    ...
    Displays the command and arguments as a list of strings that deepsh would
    run in subprocess mode. This is useful for determining how deepsh evaluates
...
def detect_xpip_alias():
    """
    Determines the correct invocation to get deepsh's pip
    """
    ...
    try:
        if ON_WINDOWS:
            ...
            # In AppImage `sys.executable` is equal to path to deepsh.AppImage file and the real python executable is in $_
            return [
...
def make_default_aliases():
    ...
    if ON_WINDOWS:
        ...
        if shutil.which("sudo", path=XSH.env.get_detyped("PATH")):
            ...
            import deepsh.platforms.winutils as winutils
