"""Implements the deepsh history backend via sqlite3."""
...
XH_SQLITE_TABLE_NAME = "deepsh_history"
XH_SQLITE_CREATED_SQL_TBL = "CREATED_SQL_TABLE"


def _xh_sqlite_get_file_name():
    envs = XSH.env
    file_name = envs.get("DEEPSH_HISTORY_SQLITE_FILE")
    if not file_name:
        data_dir = envs.get("DEEPSH_DATA_DIR")
        file_name = os.path.join(data_dir, "deepsh-history.sqlite")
...
def _xh_sqlite_get_count(cursor, sessionid=None):
    sql = "SELECT count(*) FROM deepsh_history "
    params = []
...
def _xh_sqlite_get_records(cursor, sessionid=None, limit=None, newest_first=False):
    sql = "SELECT inp, tsb, rtn, frequency, cwd FROM deepsh_history "
    params = []
    if sessionid is not None:
        sql += "WHERE sessionid = ? "
...
def _xh_sqlite_delete_records(cursor, size_to_keep):
    ...
    sql += "SELECT tsb FROM deepsh_history ORDER BY tsb DESC "
    ...
    if not result:
        return
    max_tsb = result[0]
    sql = "DELETE FROM deepsh_history WHERE tsb < ?"
...
def xh_sqlite_pull(filename, last_pull_time, current_sessionid):
    sql = "SELECT inp FROM deepsh_history WHERE tsb > ? AND sessionid != ? ORDER BY tsb"
...
def xh_sqlite_wipe_session(sessionid=None, filename=None):
    ...
    sql = "DELETE FROM deepsh_history WHERE sessionid = ?"
    with _xh_sqlite_get_conn(filename=filename) as conn:
        c = conn.cursor()
        _xh_sqlite_create_history_table(c)
    ...
    with _xh_sqlite_get_conn(filename=filename) as conn:
        ...
        for inp, *_ in _xh_sqlite_get_records(c):
            if pattern.match(inp):
                sql = f"DELETE FROM deepsh_history WHERE inp = '{inp}'"
                c.execute(sql)
...
class SqliteHistoryGC(threading.Thread):
    ...
    def run(self):
        ...
        if self.size is not None:
            ...
            envs = XSH.env
            hsize, units = envs.get("DEEPSH_HISTORY_SIZE")
        if units != "commands":
            print(
...
class SqliteHistory(History):
    """Deepsh history backend implemented with sqlite3."""
    ...
    def append(self, cmd):
        ...
        try:
            xh_sqlite_append_history(
                cmd,
                str(self.sessionid),
                store_stdout=envs.get("DEEPSH_STORE_STDOUT", False),
