"""Implements JSON version of deepsh history backend."""
...
def _xhj_get_data_dir():
    dir = xt.expanduser_abs_path(
        os.path.join(XSH.env.get("DEEPSH_DATA_DIR"), "history_json")
    )
...
def _xhj_get_history_files(sort=True, newest_first=False):
    ...
    data_dirs = [
        _xhj_get_data_dir(),
        XSH.env.get("DEEPSH_DATA_DIR"),  # backwards compatibility, remove in the future
    ]
    ...
    for data_dir in data_dirs:
        ...
        try:
            files += [
                ...
                if f.startswith("deepsh-") and f.endswith(".json")
            ]
        except OSError:
            if XSH.env.get("DEEPSH_DEBUG"):
                xt.print_exception(
                    f"Could not collect deepsh history json files from {data_dir}"
                )
    if sort:
        files.sort(key=lambda x: os.path.getmtime(x), reverse=newest_first)

    custom_history_file = XSH.env.get("DEEPSH_HISTORY_FILE", None)
...
class JsonHistoryGC(threading.Thread):
    ...
    def __init__(self, wait_for_shell=True, size=None, force=False, *args, **kwargs):
        ...
        May wait for shell (and for deepshrc to have been loaded) to start work.
        """
    ...
    def run(self):
        ...
        deepsh_debug = env.get("DEEPSH_DEBUG", 0)
        if self.size is None:
            hsize, units = env.get("DEEPSH_HISTORY_SIZE")
        else:
...
class JsonHistoryFlusher(threading.Thread):
    ...
    def dump(self):
        ...
        if self.at_exit:
            ...
            hist["locked"] = False
        if not XSH.env.get("DEEPSH_STORE_STDOUT", False):
...
class JsonHistory(History):
    """Deepsh history backend implemented with JSON files.
    ...
    def __init__(
        ...
        **meta,
        ...
        """Represents a deepsh session's history as an in-memory buffer that is
        ...
        filename : str, optional
            ...
            ``$DEEPSH_DATA_DIR/history_json/deepsh-{sessionid}.json``.
        ...
        if filename is None:
            ...
            self.filename = os.path.join(data_dir, f"deepsh-{self.sessionid}.json")
        else:
            self.filename = filename
        ...
        self.save_cwd = (
            ...
            else XSH.env.get("DEEPSH_HISTORY_SAVE_CWD", True)
        )
    ...
    def all_items(self, newest_first=False, **kwargs):
        """
        Returns all history as found in DEEPSH_DATA_DIR.
        ...
        for f in _xhj_get_history_files(newest_first=newest_first):
            ...
            try:
                ...
                if XSH.env.get("DEEPSH_DEBUG") > 0:
                    msg = "deepsh history file {0!r} is not valid JSON"
    ...
    def info(self):
        ...
        envs = XSH.env
        data["gc options"] = envs.get("DEEPSH_HISTORY_SIZE")
