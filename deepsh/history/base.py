"""Base class of Deepsh History backends."""
...
class HistoryEntry(types.SimpleNamespace):
    ...
    out: str
        The output of the command, if deepsh is configured to save it
    rtn: int
...
class History:
    """Deepsh history backend base class.
    ...
    def __init__(self, sessionid=None, **kwargs):
        """Represents a deepsh session's history.
    ...
    def ignore_regex(self):
        compiled_regex = None
        regex = XSH.env.get("DEEPSH_HISTORY_IGNORE_REGEX")
        if regex:
            try:
