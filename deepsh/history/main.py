"""Main entry points of the deepsh history."""
...
import deepsh.history.diff_history as xdh
...
def construct_history(backend=None, **kwargs) -> "History":
    ...
    env = XSH.env
    backend = backend or env.get("DEEPSH_HISTORY_BACKEND", "json")
    ...
    try:
        ...
        xt.print_exception(
            ...
            f"Set $DEEPSH_HISTORY_BACKEND='dummy' to disable history.\n"
            f"History disabled."
        )
        return DummyHistory()
...
def _xh_get_history(
    ...
    *,
    ...
    Parameters
    ----------
    session: {'session', 'all', 'deepsh', 'bash', 'zsh'}
...
class HistoryAlias(xcli.ArgParserAlias):
    ...
    def show(
        ...
        ----------
        session:
            The history session to get. (all is an alias for deepsh)
        slices:
    ...
    def pull(show_commands=False, _stdout=None):
        ...
        if hist.pull.__module__ == "deepsh.history.base":
            backend = XSH.env.get("DEEPSH_HISTORY_BACKEND", "unknown")
            print(
    ...
    def transfer(
        ...
        target : --target, -t
            Name of the target history backend. (default: $DEEPSH_HISTORY_BACKEND)
