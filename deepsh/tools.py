...
# adding imports from further deepsh modules is discouraged to avoid circular
# dependencies
from deepsh import __version__
from deepsh.lib.lazyasd import LazyDict, LazyObject, lazyobject
...
class DeepshError(Exception):
    pass


class DeepshCalledProcessError(DeepshError, subprocess.CalledProcessError):
    """Raised when there's an error with a called process

    Inherits from DeepshError and subprocess.CalledProcessError, catching
...
def decode_bytes(b):
    """Tries to decode the bytes using DEEPSH_ENCODING if available,
    otherwise using sys.getdefaultencoding().
    """
    env = xsh.env or os_environ
    enc = env.get("DEEPSH_ENCODING") or DEFAULT_ENCODING
    err = env.get("DEEPSH_ENCODING_ERRORS") or "strict"
...
def get_line_continuation():
    ...
    if ON_WINDOWS:
        ...
        if env.get("DEEPSH_INTERACTIVE", False):
            return " \\"
...
def print_warning(msg):
    ...
    manually_set_trace, show_trace = _get_manual_env_var("DEEPSH_SHOW_TRACEBACK", False)
    manually_set_logfile, log_file = _get_manual_env_var("DEEPSH_TRACEBACK_LOGFILE")
    ...
    if show_trace:
        # notify user about DEEPSH_TRACEBACK_LOGFILE if it has
        # not been set manually
        if not manually_set_logfile:
            sys.stderr.write(
                "deepsh: To log full traceback to a file set: "
                "$DEEPSH_TRACEBACK_LOGFILE = <filename>\n"
            )
...
def print_exception(msg=None, exc_info=None, source_msg=None):
    ...
    _, debug_level = _get_manual_env_var("DEEPSH_DEBUG", 0)

    # the interal state of the parsers stack is
    # not helpful in normal operation (DEEPSH_DEBUG == 0).
    # this is also done to be consistent with python
    is_syntax_error = issubclass(exc_info[0], SyntaxError)

    # DeepshErrors don't show where in the users code they occured
    # (most are reported deeper in the callstack, e.g. see procs/pipelines.py),
    # but only show non-helpful deepsh internals.
    # These are only relevent when developing/debugging deepsh itself.
    # Therefore, dont print these traces until this gets overhauled.
    is_deepsh_error = exc_info[0] in (DeepshError, DeepshCalledProcessError)

    # hide unhelpful traces if not debugging
    hide_stacktrace = debug_level == 0 and (is_syntax_error or is_deepsh_error)
    if hide_stacktrace:
        limit = 0
        chain = False

    if xsh.env.get("DEEPSH_SHOW_TRACEBACK", False):
        """
        This moved under ``DEEPSH_SHOW_TRACEBACK`` because it looks that python's
    ...
    if not show_trace:
        ...
        if not xsh.env.get("DEEPSH_SHOW_TRACEBACK") and xsh.env.get(
            "RAISE_SUBPROC_ERROR"
        ):
...
def display_colored_error_message(exc_info, strip_deepsh_error_types=True, limit=None):
    no_trace_and_raise_subproc_error = not xsh.env.get(
        "DEEPSH_SHOW_TRACEBACK"
    ...
    _, interactive = _get_manual_env_var("DEEPSH_INTERACTIVE", 0)
    _, color_results = _get_manual_env_var("COLOR_RESULTS", 0)
    shell_loaded = hasattr(xsh.shell, "shell")  # do not color traceback in deepshrc file
...
def display_error_message(exc_info, strip_deepsh_error_types=True):
    """
    Prints the error message of the given sys.exc_info() triple on stderr.
    """
    exc_type, exc_value, exc_traceback = exc_info
    exception_only = traceback.format_exception_only(exc_type, exc_value)
    if exc_type is DeepshError and strip_deepsh_error_types:
...
def is_logfile_opt(x):
    """
    Checks if x is a valid $DEEPSH_TRACEBACK_LOGFILE option. Returns False
...
def to_logfile_opt(x):
    """Converts a $DEEPSH_TRACEBACK_LOGFILE option to either a str containing
    ...
    if is_logfile_opt(x):
        ...
        sys.stderr.write(
            "deepsh: $DEEPSH_TRACEBACK_LOGFILE must be a "
...
def logfile_opt_to_str(x):
    """
    Detypes a $DEEPSH_TRACEBACK_LOGFILE option.
    """
    if x is None:
...
def is_tok_color_dict(x):
    ...
    from deepsh.shells.ptk_shell import _style_from_pygments_dict
...
def format_color(string, **kwargs):
    ...
    if hasattr(xsh.shell, "shell"):
        ...
        from deepsh.ansi_colors import ansi_partial_color_format

        style = xsh.env.get("DEEPSH_COLOR_STYLE")
...
def register_custom_style(
    ...
    if pygments_version_info():
        from deepsh.pyghooks import register_custom_pygments_style
    ...
    from deepsh.ansi_colors import register_custom_ansi_style
...
def format_std_prepost(template, env=None):
    ...
    if xsh.shell is None:
        # shell hasn't fully started up (probably still in deepshrc)
        from deepsh.ansi_colors import ansi_partial_color_format
        from deepsh.prompt.base import PromptFormatter

        pf = PromptFormatter()
        s = pf(template)
        style = env.get("DEEPSH_COLOR_STYLE")
...
def expandvars(path):
    ...
    if isinstance(path, bytes):
        path = path.decode(
            encoding=env.get("DEEPSH_ENCODING"), errors=env.get("DEEPSH_ENCODING_ERRORS")
        )
...
def ensure_timestamp(t, datetime_format=None):
    ...
    if datetime_format is None:
        datetime_format = xsh.env["DEEPSH_DATETIME_FORMAT"]
...
def format_datetime(dt):
    """Format datetime object to string base on $DEEPSH_DATETIME_FORMAT Env."""
    format_ = xsh.env["DEEPSH_DATETIME_FORMAT"]
