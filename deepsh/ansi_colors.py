...
from deepsh.lib.lazyasd import LazyDict, lazyobject
from deepsh.platform import HAS_PYGMENTS
from deepsh.tools import FORMATTER
...
def _ansi_partial_color_format_main(template, style="default", cmap=None, hide=False):
    cmap = _ensure_color_map(style=style, cmap=cmap)
    overrides = XSH.env["DEEPSH_STYLE_OVERRIDES"]
    if overrides:
...
def _color_name_from_ints(ints, background=False, prefix=None):
    name = find_closest_color(ints, BASE_DEEPSH_COLORS)
    if background:
...
def ansi_style_by_name(name):
    ...
    from deepsh.pygments_cache import get_style_by_name
