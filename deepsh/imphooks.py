"""Import hooks for importing deepsh source files.
...
from deepsh.tools import print_warning
...
class DeepshImportHook(MetaPathFinder, SourceLoader):
    """Implements the import hook for deepsh source files."""
    ...
    def find_spec(self, fullname, path, target=None):
        """Finds the spec for a deepsh module if it exists."""
        dot = "."
        spec = None
    ...
    def create_module(self, spec):
        """Create a deepsh module with the appropriate attributes."""
    ...
    def get_code(self, fullname):
        """Gets the code object for a deepsh file."""
        filename = self.get_filename(fullname)
        if filename is None:
            msg = f"deepsh file {fullname!r} could not be found"
...
def _should_dispatch_deepsh_import_event_loader():
...
class DeepshImportEventHook(MetaPathFinder):
    """Implements the import hook for firing deepsh events on import."""
    ...
    def find_spec(self, fullname, path, target=None):
        ...
        dispatch_load = _should_dispatch_deepsh_import_event_loader()
        if npre > 0:
        ...
        if dispatch_load and spec is not None and hasattr(spec.loader, "create_module"):
            spec.loader = DeepshImportEventLoader(spec.loader)
        return spec
...
class DeepshImportEventLoader(Loader):
...
def install_import_hooks(execer=ARG_NOT_PRESENT):
    """
    Install Deepsh import hooks in ``sys.meta_path`` in order for ``.xsh`` files
    to be importable and import events to be fired.

    Can safely be called many times, will be no-op if deepsh import hooks are
    already present.
    """
    ...
    for hook in sys.meta_path:
        if isinstance(hook, DeepshImportHook):
            found_imp = True
        elif isinstance(hook, DeepshImportEventHook):
            found_event = True
    if not found_imp:
        sys.meta_path.append(DeepshImportHook(execer))
    if not found_event:
        sys.meta_path.insert(0, DeepshImportEventHook())
