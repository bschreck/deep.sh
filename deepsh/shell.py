...
from deepsh.tools import DeepshError, is_class, print_exception, simple_random_choice
...
* ``out``: If deepsh stores command output, this is the output
...
def transform_command(src, show_diff=True):
    ...
    debug_level = XSH.env.get("DEEPSH_DEBUG")
...
class Shell:
    ...
    def choose_shell_type(init_shell_type=None, env=None):
        ...
        if shell_type is None and env:
            ...
            if shell_type == "none":
                # This bricks interactive deepsh
    ...
    def construct_shell_cls(backend, **kwargs):
        ...
        if is_class(backend):
            ...
            `echo 'echo 1' | deepsh -i` and it's not working with `TERM=dumb` (#5462 #5517)
            ...
            is_stdin_to_interactive = (
                XSH.env.get("DEEPSH_INTERACTIVE", False) and not sys.stdin.isatty()
            )

            if backend == "none":
                from deepsh.shells.base_shell import BaseShell as cls
            elif backend == "prompt_toolkit" or is_stdin_to_interactive:
                from deepsh.shells.ptk_shell import PromptToolkitShell as cls
            elif backend == "readline":
                from deepsh.shells.readline_shell import ReadlineShell as cls
            elif backend == "dumb":
                from deepsh.shells.dumb_shell import DumbShell as cls
            else:
                raise DeepshError(f"{backend} is not recognized as a shell type")
        return cls(**kwargs)
    ...
    def __init__(self, execer, ctx=None, shell_type=None, **kwargs):
        ...
        ----------
        execer : Execer
            An execer instance capable of running deepsh code.
        ...
        if env.get("DEEPSH_INTERACTIVE"):
            XSH.history = hist = xhm.construct_history(
                env=env.detype(),
                ts=[time.time(), None],
                locked=True,
                filename=env.get("DEEPSH_HISTORY_FILE", None),
            )
            env["DEEPSH_HISTORY_FILE"] = hist.filename
        else:
            XSH.history = hist = DummyHistory()
            env["DEEPSH_HISTORY_FILE"] = None
