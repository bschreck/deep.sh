...
import deepsh.wizard as wiz
from deepsh import __version__ as DEEPSH_VERSION
from deepsh.built_ins import XSH
from deepsh.cli_utils import Arg, ArgParserAlias
from deepsh.events import events
from deepsh.foreign_shells import CANON_SHELL_NAMES
from deepsh.lib.lazyasd import lazyobject
from deepsh.parsers import ply
...
from deepsh.prompt.base import is_template_string
...
HR = "'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'"
WIZARD_HEAD = f"""
          {{BOLD_WHITE}}Welcome to the deepsh configuration wizard!{{RESET}}
          {{YELLOW}}------------------------------------------{{RESET}}
...
{HR}
                      ...
                      {{YELLOW}}-------------------{{RESET}}
The deepsh shell has the ability to interface with Bash or zsh
...
def _dump_config_foreign_shell(path, value):
    ...
    if cmd[0] == "source-foreign":
        cmd.append(shell)
    cmd.append('"echo loading deepsh foreign shell"')
    return " ".join(cmd)
...
def make_config_wizard(default_file=None, confirm=False, no_wizard_file=None):
    ...
    if confirm:
        q = (
            "Would you like to run the deepsh configuration wizard now?\n\n"
...
def _wizard(
    ...
    -------
    rcfile : -f, --file
        config file location, default=$DEEPSHRC
    confirm : -c, --confirm
    ...
    def force_hide():
        if env.get("DEEPSH_STORE_STDOUT") and hasattr(shell, "_force_hide"):
...
def _info(
    ...
    """
    env = XSH.env
    data: list[tp.Any] = [("deepsh", DEEPSH_VERSION)]
    hash_, date_ = githash()
    if hash_:
    ...
    data.extend(
        [
            ...
            ("history backend", env.get("DEEPSH_HISTORY_BACKEND")),
    ...
    data.extend(
        [
            ...
            ("deepsh encoding", env.get("DEEPSH_ENCODING")),
    ...
    envs = {
        ...
        "DEEPSH_CAPTURE_ALWAYS": None,
        "DEEPSH_SUBPROC_OUTPUT_FORMAT": None,
        "THREAD_SUBPROCS": None,
        "ENABLE_ASYNC_PROMPT": True,
        "ENABLE_COMMANDS_CACHE": False,
        "DEEPSH_CACHE_EVERYTHING": True,
        "DEEPSH_CACHE_SCRIPTS": None,
    }
...
def _styles(to_json=False, _stdout=None):
    """Prints available deepsh color styles
    ...
    """
    env = XSH.env
    curr = env.get("DEEPSH_COLOR_STYLE")
...
def _tok_colors(cmap, cols):
    from deepsh.style_tools import Color
...
def _colors(
    ...
    style_stash = XSH.env["DEEPSH_COLOR_STYLE"]

    if style is not None:
        if style not in color_style_names():
            print(f"Invalid style: {style}")
            return
        XSH.env["DEEPSH_COLOR_STYLE"] = style
    ...
    print_color(s)
    XSH.env["DEEPSH_COLOR_STYLE"] = style_stash
...
def _web(
    ...
    from deepsh.webconfig import main
...
WELCOME_MSG = [
    "",
    ("Welcome to the deepsh shell {version}", "^", " "),
    "",
...
def print_welcome_screen():
    ...
    subst = dict(tagline=random.choice(list(TAGLINES)), version=DEEPSH_VERSION)
        ...
        if elem == "[SHELL_TYPE_WARNING]":
            if shell_type != "prompt_toolkit":
                print_color(
                    ...
                    f"  {{GREEN}}xpip{{RESET}} install -U 'deepsh[full]'"
                )
            continue
