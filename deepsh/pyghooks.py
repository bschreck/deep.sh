...
from deepsh.color_tools import (
    BASE_DEEPSH_COLORS,
    RE_BACKGROUND,
    RE_DEEPSH_COLOR,
    find_closest_color,
...
from deepsh.lib.lazyimps import html, os_listxattr, terminal256
...
from deepsh.pygments_cache import add_custom_style, get_style_by_name
from deepsh.style_tools import DEFAULT_STYLE_DICT, norm_name
...
def color_name_to_pygments_code(name, styles):
    """Converts a deepsh color name to a pygments color code."""
    token = getattr(Color, norm_name(name))
    if token in styles:
        return styles[token]
    m = RE_DEEPSH_COLOR.match(name)
    if m is None:
...
def color_token_by_name(xc: tuple, styles=None) -> _TokenType:
    """Returns (color) token corresponding to Deepsh color tuple, side effect: defines token is defined in styles"""
    if not styles:
        try:
...
class DeepshStyle(Style):
    """A deepsh pygments style that will dispatch to the correct color map
    ...
    def style_name(self, value):
        ...
        if value not in STYLES:
            try:  # loading style dynamically
                ...
                value = "default"
                XSH.env["DEEPSH_COLOR_STYLE"] = value
        cmap = STYLES[value]
        if value == "default":
            self._smap = DEEPSH_BASE_STYLE.copy()
        else:
            try:
        ...
        for file_type, deepsh_color in XSH.env.get("LS_COLORS", {}).items():
            color_token = color_token_by_name(deepsh_color, self.styles)
...
def deepsh_style_proxy(styler):
    """Factory for a proxy class to a deepsh style."""
...
def make_pygments_style(palette):
    ...
    for name, t in BASE_DEEPSH_COLORS.items():
...
def _monkey_patch_pygments_codes():
    ...
    if not getattr(pygments.console, "_deepsh_patched", False):
        patched_codes = {}
        ...
        pygments.console._deepsh_patched = True
...
@lazyobject
def DeepshTerminal256Formatter():
    if (
        ptk_version_info()
    ...
    class DeepshTerminal256FormatterProxy(terminal256.Terminal256Formatter):
        """Proxy class for deepsh terminal256 formatting that understands.
        deepsh color tokens.
        """
    ...
    return DeepshTerminal256FormatterProxy


@lazyobject
def DeepshHtmlFormatter():
    ...
    class DeepshHtmlFormatterProxy(html.HtmlFormatter):
        """Proxy class for deepsh HTML formatting that understands.
        deepsh color tokens.
        """
    ...
    return DeepshHtmlFormatterProxy
...
class DeepshLexer(Python3Lexer):
    """Deepsh console lexer for pygments."""

    name = "Deepsh lexer"
    aliases = ["deepsh", "xsh"]
    filenames = ["*.xsh", "*deepshrc"]
...
class DeepshConsoleLexer(DeepshLexer):
    """Deepsh console lexer for pygments."""

    name = "Deepsh console lexer"
    aliases = ["deepshcon"]
    filenames: list[str] = []
