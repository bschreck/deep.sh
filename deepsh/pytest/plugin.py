...
from deepsh import commands_cache
from deepsh.aliases import Aliases
from deepsh.built_ins import XSH, DeepshSession
...
class XshFunction(pytest.Item):
    ...
    def repr_failure(self, excinfo, **_):
        ...
        formatted_tb.insert(0, "deepsh execution failed\n")
        formatted_tb.append(f"{excinfo.type.__name__}: {excinfo.value}")
        return "".join(formatted_tb)

    def reportinfo(self):
        return self.path, 0, f"deepsh test: {self.name}"


@pytest.fixture
def source_path():
    """Get the deepsh source path."""
    pwd = os.path.dirname(__file__)
    return os.path.dirname(pwd)


@pytest.fixture
def deepsh_execer(monkeypatch, deepsh_session):
    """Initiate the Execer with a mocked nop `load_builtins`"""
    yield deepsh_session.execer


@pytest.fixture
def deepsh_execer_exec(deepsh_execer):
    def factory(input, **kwargs):
        deepsh_execer.exec(input, **kwargs)
        return True
...
@pytest.fixture
def deepsh_execer_parse(deepsh_execer):
    def factory(input):
...
def session_os_env():
    ...
    from deepsh.environ import Env, default_env
...
def env(tmp_path, session_env):
    ...
    initial_vars = {"DEEPSH_DATA_DIR": str(tmp_path), "DEEPSH_CACHE_DIR": str(tmp_path)}
...
@pytest.fixture
def deepsh_session(deepsh_events, session_execer, os_env, monkeypatch):
    """a fixture to use where DeepshSession is fully loaded without any mocks"""
...
@pytest.fixture
def mock_deepsh_session(monkeypatch, deepsh_events, deepsh_session, env):
    """Mock out most of the builtins deepsh attributes."""
    ...
    def factory(*attrs_to_skip: str):
        ...
        for attr, val in [
            ...
            if attr in attrs_to_skip:
                continue
            monkeypatch.setattr(deepsh_session, attr, val)
        ...
        for attr, val in [
            ...
            ("events", deepsh_events),
        ]:
            # attributes to builtins are dynamicProxy and should pickup the following
            monkeypatch.setattr(deepsh_session.builtins, attr, val)

        session.append(deepsh_session)
        return deepsh_session
...
@pytest.fixture
def xession(mock_deepsh_session) -> DeepshSession:
    """Mock out most of the builtins deepsh attributes."""
    return mock_deepsh_session()


@pytest.fixture
def xsh_with_aliases(mock_deepsh_session) -> DeepshSession:
    """Deepsh mock-session with default set of aliases"""
    return mock_deepsh_session("aliases")


@pytest.fixture
def xsh_with_env(mock_deepsh_session) -> DeepshSession:
    """Deepsh mock-session with os.environ"""
    return mock_deepsh_session("env")
...
@pytest.fixture
def ptk_shell(deepsh_execer):
    from prompt_toolkit.input import create_pipe_input
    from prompt_toolkit.output import DummyOutput

    from deepsh.shells.ptk_shell import PromptToolkitShell

    out = DummyOutput()
    with create_pipe_input() as inp:
        shell = PromptToolkitShell(
            execer=deepsh_execer, ctx={}, ptk_args={"input": inp, "output": out}
        )
        yield inp, out, shell


@pytest.fixture
def readline_shell(deepsh_execer, tmpdir, mocker):
    ...
    shell = ReadlineShell(execer=deepsh_execer, ctx={}, stdin=inp, stdout=out)
