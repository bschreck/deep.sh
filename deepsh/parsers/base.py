"""Implements the base deepsh parser."""
...
from deepsh.parsers.ast import has_elts, load_attribute_chain, deepsh_call
from deepsh.parsers.context_check import check_contexts
from deepsh.parsers.fstring_adaptor import FStringAdaptor
from deepsh.parsers.lexer import Lexer, LexToken
from deepsh.parsers.ply import yacc
from deepsh.parsers.tokenize import SearchPath, StringPrefix
...
def deepsh_help(x, lineno=None, col=None):
    """Creates the AST node for calling the __deepsh__.help() function."""
    return deepsh_call("__deepsh__.help", [x], lineno=lineno, col=col)


def deepsh_superhelp(x, lineno=None, col=None):
    """Creates the AST node for calling the __deepsh__.superhelp() function."""
    return deepsh_call("__deepsh__.superhelp", [x], lineno=lineno, col=col)
...
class BaseParser:
    """A base class that parses the deepsh language."""

    def __init__(
        self,
        yacc_optimize=True,
        yacc_table="deepsh.parser_table",
        yacc_debug=False,
        outputdir=None,
    ):
        """Parameters
    ...
    def parse(self, s, filename="<code>", mode="exec", debug_level=0):
        """Returns an abstract syntax tree of deepsh code.
    ...
    def deepsh_pathsearch(self, pattern, pymode=False, lineno=None, col=None):
        """Creates the AST node for calling the __deepsh__.pathsearch() function.
    ...
    def p_with_bang_stmt_single_suite(self, p):
        ...
        gblcall = deepsh_call("globals", [], lineno=l, col=c)
        loccall = deepsh_call("locals", [], lineno=l, col=c)
        margs = [expr, p4, gblcall, loccall]
        p3.context_expr = deepsh_call("__deepsh__.enter_macro", margs, lineno=l, col=c)
    ...
    def p_with_bang_stmt_many_suite(self, p):
        ...
        for item in items:
            ...
            item.context_expr = deepsh_call(
                "__deepsh__.enter_macro", margs, lineno=l, col=c
            )
    ...
    def apply_trailers(self, leader, trailers):
        ...
        for trailer in trailers:
            if isinstance(
                ...
                p0 = deepsh_call("__deepsh__.call_macro", margs, lineno=l, col=c)
            elif isinstance(trailer, str):
                if trailer == "?":
                    p0 = deepsh_help(leader, lineno=leader.lineno, col=leader.col_offset)
                elif trailer == "??":
                    p0 = deepsh_superhelp(
    ...
    def p_string_literal(self, p):
        ...
        if "p" in prefix and "f" in prefix:
            ...
            p[0] = deepsh_call(
                "__deepsh__.path_literal", [s], lineno=p1.lineno, col=p1.lexpos
            )
        elif "p" in prefix:
    ...
    def p_string_literal_list(self, p):
        ...
        if len(p) == 3:
            ...
            if p1_type == p2_type:
                if p1_type == "path":
                    # the nodes are function calls to __deepsh__.path_literal()
    ...
    def _dollar_rules(self, p):
        """These handle the special deepsh $ shell atoms by looking up
        in a special __deepsh__.env dictionary injected in the __builtin__.
        """
        lenp = len(p)
        p1, p2 = p[1], p[2]
        ...
        if lenp == 3:  # $NAME
            ...
            xenv = load_attribute_chain("__deepsh__.env", lineno=lineno, col=col)
            ...
            p0 = deepsh_call(
                "__deepsh__.subproc_captured_stdout", p2, lineno=lineno, col=col
            )
        elif p1 == "!(":
            p0 = deepsh_call(
                "__deepsh__.subproc_captured_object", p2, lineno=lineno, col=col
            )
        elif p1 == "![":
            p0 = deepsh_call(
                "__deepsh__.subproc_captured_hiddenobject", p2, lineno=lineno, col=col
            )
        elif p1 == "$[":
            p0 = deepsh_call("__deepsh__.subproc_uncaptured", p2, lineno=lineno, col=col)
        else:
            raise AssertionError()
    ...
    def _envvar_by_name(self, var, lineno=None, col=None):
        """Looks up a deepsh variable by name."""
        xenv = load_attribute_chain("__deepsh__.env", lineno=lineno, col=col)
    ...
    def p_subproc_atoms_subshell(self, p):
        ...
        subcmd = subcmd.strip() + "\n"
        p0 = [
            ast.const_str(s="deepsh", lineno=l, col_offset=c),
    ...
    def p_subproc_atom_uncaptured(self, p):
        ...
        p1 = p[1]
        p0 = deepsh_call(
            "__deepsh__.subproc_uncaptured", args=p[2], lineno=p1.lineno, col=p1.lexpos
        )
    ...
    def p_subproc_atom_captured_stdout(self, p):
        ...
        p1 = p[1]
        p0 = deepsh_call(
            "__deepsh__.subproc_captured_stdout",
            args=p[2],
            lineno=p1.lineno,
            col=p1.lexpos,
        )
    ...
    def p_subproc_atom_pyeval(self, p):
        ...
        p1 = p[1]
        p0 = deepsh_call(
            "__deepsh__.list_of_strs_or_callables",
            [p[2]],
            lineno=p1.lineno,
            col=p1.lexpos,
        )
    ...
    def p_subproc_atom_subproc_inject(self, p):
        ...
        p1, p2 = p[1], p[2]
        p0 = deepsh_call(
            "__deepsh__.subproc_captured_inject", p2, lineno=p1.lineno, col=p1.lexpos
        )
        p0._cliarg_action = "extend"
    ...
    def p_subproc_atom_re(self, p):
        ...
        p0 = self.deepsh_pathsearch(p[1], pymode=False, lineno=self.lineno, col=self.col)
        p0._cliarg_action = "extend"
        p[0] = p0
    ...
    def p_subproc_atom_str(self, p):
        ...
        if hasattr(p[1], "is_raw") and p[1].is_raw:
            ...
            p0 = deepsh_call(
                "__deepsh__.expand_path", args=[p[1]], lineno=self.lineno, col=self.col
            )
        p0._cliarg_action = "append"
        p[0] = p0
    ...
    def p_subproc_atom_arg(self, p):
        ...
        if isinstance(p1, list):
            ...
            p0 = deepsh_call(
                "__deepsh__.list_of_list_of_strs_outer_product",
            ...
            p0 = deepsh_call(
                "__deepsh__.glob", args=[p1], lineno=p1.lineno, col=p1.col_offset
            )
            p0._cliarg_action = "extend"
        else:
            # literal str argument
            p0 = deepsh_call(
                "__deepsh__.expand_path", args=[p1], lineno=p1.lineno, col=p1.col_offset
            )
            p0._cliarg_action = "append"
        p[0] = p0
