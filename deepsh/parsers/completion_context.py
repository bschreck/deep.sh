"""Implements the deepsh (tab-)completion context parser.
...
from deepsh.parsers.base import Location, raise_parse_error
from deepsh.parsers.lexer import Lexer
from deepsh.parsers.ply import yacc
from deepsh.tools import check_for_partial_string, get_line_continuation
...
class CommandContext(NamedTuple):
    ...
    This affects the completion's behaviour - see ``Completer.complete`` in ``deepsh/completer.py``.
    """
...
class CompletionContextParser:
    ...
    def __init__(
        self,
        yacc_optimize=True,
        yacc_table="deepsh.completion_parser_table",
        debug=False,
        outputdir=None,
    ):
        self.cursor = 0
    ...
    def try_parse_string_literal(raw_arg: str) -> Optional[CommandArg]:
        ...
        if startix != 0 or endix not in (
            ...
            # deepsh won't treat it as a string literal
            return None
        else:
            if endix is None:
                # no closing quote
