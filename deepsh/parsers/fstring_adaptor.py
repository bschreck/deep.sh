"""Implements helper class for parsing Deepsh syntax within f-strings."""
...
def RE_FSTR_FIELD_WRAPPER():
    return re.compile(r"(__deepsh__\.eval_fstring_field\((\d+)\))\s*[^=]")
...
class FStringAdaptor:
    """Helper for parsing Deepsh syntax within f-strings."""

    def __init__(self, fstring, prefix, filename=None):
        """Parses an f-string containing special Deepsh syntax and returns
    ...
    def _patch_special_syntax(self):
        ...
        deepsh expressions as its field values and substitues them for
        a call to __deepsh__.eval_fstring_field as needed.
        """
        ...
        while True:
            ...
            deepsh_field = (error_expr, self.filename if self.filename else None)
            field_id = id(deepsh_field)
            self.fields[field_id] = deepsh_field
            eval_field = f"__deepsh__.eval_fstring_field({field_id})"
    ...
    def _fix_eval_field_params(self):
        ...
        for node in ast.walk(self.res):
            if not (
                ...
                and node.func.value.id == "__deepsh__"
