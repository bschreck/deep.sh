...
from deepsh.tools import find_next_break, get_logical_line, subproc_toks
...
def deepsh_call(name, args, lineno=None, col=None):
    """Creates the AST node for calling a function of a given name.
    Functions names may contain attribute access, e.g. __deepsh__.env.
    """
    return Call(
...
class CtxAwareTransformer(NodeTransformer):
    """Transforms a deepsh AST based to use subprocess calls when
...
def _getblockattr(name, lineno, col):
    """calls getattr(name, '__deepsh_block__', False)."""
    return deepsh_call(
        "getattr",
        args=[
            Name(id=name, ctx=Load(), lineno=lineno, col_offset=col),
            const_str(s="__deepsh_block__", lineno=lineno, col_offset=col),
