...
import deepsh.lib.lazyasd as xl
import deepsh.lib.lazyimps as xli
import deepsh.platform as xp
import deepsh.procs.jobs as xj
import deepsh.tools as xt
from deepsh.built_ins import XSH
from deepsh.procs.executables import locate_executable
from deepsh.procs.pipelines import (
    STDOUT_CAPTURE_KINDS,
    CommandPipeline,
    HiddenCommandPipeline,
    resume_process,
)
from deepsh.procs.posix import PopenThread
from deepsh.procs.proxies import ProcProxy, ProcProxyThread
from deepsh.procs.readers import ConsoleParallelReader
...
def get_script_subproc_command(fname, args):
    ...
    # deepsh is the default interpreter
    if m is None:
        interp = ["deepsh"]
    else:
...
def safe_open(fname, mode, buffering=-1):
    """Safely attempts to open a file in for deepsh subprocs."""
    # file descriptors
    try:
        return open(fname, mode, buffering=buffering)
    except PermissionError as ex:
        raise xt.DeepshError(f"deepsh: {fname}: permission denied") from ex
    except FileNotFoundError as ex:
        raise xt.DeepshError(f"deepsh: {fname}: no such file or directory") from ex
    except Exception as ex:
        raise xt.DeepshError(f"deepsh: {fname}: unable to open file") from ex
    ...
    if dest.startswith("&"):
        try:
            ...
            if loc is None:
                ...
                raise xt.DeepshError(e)
        except (ValueError, xt.DeepshError):
            raise
        except Exception:
            pass
    mode = _MODES.get(mode, None)
    if mode == "r" and (len(orig) > 0 or len(dest) > 0):
        raise xt.DeepshError(f"Unrecognized redirection command: {r}")
...
class SubprocSpec:
    ...
    def __init__(
        ...
        self.output_format = XSH.env.get("DEEPSH_SUBPROC_OUTPUT_FORMAT", "stream_lines")
    ...
    def _run_binary(self, kwargs):
        if not self.cmd[0]:
            raise xt.DeepshError("deepsh: subprocess mode: command is empty")
        bufsize = 1
        try:
            ...
            e = "deepsh: subprocess mode: permission denied: {0}"
            raise xt.DeepshError(e.format(self.cmd[0])) from ex
        except FileNotFoundError as ex:
            cmd0 = self.cmd[0]
    ...
    def prep_env_subproc(self, kwargs):
        ...
        with XSH.env.swap(self.env) as env:
            denv = env.detype()
        if xp.ON_WINDOWS:
            # Over write prompt variable as deepsh's $PROMPT does
    ...
    def prep_preexec_fn(self, kwargs, pipeline_group=None):
        ...
        kwargs["preexec_fn"] = deepsh_preexec_fn
    ...
    def resolve_stack(self):
        ...
        if not stack[3][3].startswith("test_"):
            assert stack[3][3] == "run_subproc", "deepsh stack has changed!"
        del stack[:5]
        self.stack = stack
...
def _last_spec_update_threading(last: SubprocSpec):
    ...
    threadable = (
        ...
        and (captured != "hiddenobject" or env.get("DEEPSH_CAPTURE_ALWAYS"))
...
def _update_proc_alias_threadable(proc):
    threadable = XSH.env.get("THREAD_SUBPROCS") and getattr(
        proc.alias, "__deepsh_threadable__", True
    )
...
def _update_proc_alias_captured(proc):
    proc.captured = getattr(proc.alias, "__deepsh_capturable__", proc.captured)


def _trace_specs(trace_mode, specs, cmds, captured):
    """Show information about specs."""
    tracer = XSH.env.get("DEEPSH_TRACE_SUBPROC_FUNC", None)
    if callable(tracer):
...
def cmds_to_specs(cmds, captured=False, envs=None):
    ...
    for i, redirect in enumerate(redirects):
        if redirect == "|":
            ...
            raise xt.DeepshError(f"unrecognized redirect {redirect!r}")

    # Apply boundary conditions
    if not XSH.env.get("DEEPSH_CAPTURE_ALWAYS"):
...
def _set_specs_capture_always(specs_to_capture):
    """Set DEEPSH_CAPTURE_ALWAYS for all specs."""
    for spec in specs_to_capture:
        if spec.env is None:
            spec.env = {"DEEPSH_CAPTURE_ALWAYS": True}
        else:
            spec.env.setdefault("DEEPSH_CAPTURE_ALWAYS", True)


def _shell_set_title(cmds):
    if XSH.env.get("DEEPSH_INTERACTIVE") and XSH.shell is not None:
...
def run_subproc(cmds, captured=False, envs=None):
    ...
    if trace_mode := XSH.env.get("DEEPSH_TRACE_SUBPROC", False):
