"""Job control for the deepsh shell."""
...
from deepsh.platform import FD_STDERR, LIBC, ON_CYGWIN, ON_DARWIN, ON_MSYS, ON_WINDOWS
from deepsh.tools import get_signal_name, on_main_thread, unthreadable
...
def proc_untraced_waitpid(proc, hang, task=None, raise_child_process_error=False):
    ...
    See also ``deepsh.tools.describe_waitpid_status()``.
...
def add_job(info):
    ...
    get_jobs()[num] = info
    if info["bg"] and XSH.env.get("DEEPSH_INTERACTIVE"):
        print_one_job(num)
        ...
        if get_jobs():
            ...
            if _last_exit_time and last_cmd_start and _last_exit_time > last_cmd_start:
                ...
                if XSH.env["SHELL_TYPE"] != "prompt_toolkit":
                    ...
                    # newline
                    print()
                print(f"deepsh: {msg}", file=sys.stderr)
...
def hup_all_jobs():
    """
    Send SIGHUP to all child processes (called when exiting deepsh).
    """
    _clear_dead_jobs()
