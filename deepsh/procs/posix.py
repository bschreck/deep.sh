...
from deepsh.procs.jobs import proc_untraced_waitpid
from deepsh.procs.readers import (
...
class PopenThread(threading.Thread):
    ...
    def __init__(self, *args, stdin=None, stdout=None, stderr=None, **kwargs):
        ...
        self.store_stdin = env.get("DEEPSH_STORE_STDIN")
        self.timeout = env.get("DEEPSH_PROC_FREQUENCY")
        ...
        if uninew:
            self.encoding = enc = env.get("DEEPSH_ENCODING")
            self.encoding_errors = err = env.get("DEEPSH_ENCODING_ERRORS")
    ...
    def _enable_cbreak_stdin(self):
        ...
        try:
            ...
            # deepsh's tty device, such as in testing.
            self.stdin_mode = None
            return
        new = self.stdin_mode[:]
