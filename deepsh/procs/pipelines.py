...
from deepsh.procs.readers import ConsoleParallelReader, NonBlockingFDReader, safe_fdclose
...
def update_process_group(pipeline_group, background):
    if not xp.ON_POSIX:
        return False
    env = XSH.env
    if not env.get("DEEPSH_INTERACTIVE"):
        return False
    if background:
        return True
...
class CommandPipeline:
    ...
    def __init__(self, specs):
        ...
        if xp.ON_POSIX and not xt.on_main_thread():
            ...
            # process group as deepsh. This fixes case 2 of issue #4277, where
    ...
    def __repr__(self):
        debug = XSH.env.get("DEEPSH_DEBUG", False)
    ...
    def iterraw(self):
        ...
        if proc is None:
            return
        timeout = XSH.env.get("DEEPSH_PROC_FREQUENCY")
    ...
    def itercheck(self):
        ...
        if self.returncode:
            ...
            raise xt.DeepshCalledProcessError(
    ...
    def stream_stderr(self, lines):
        ...
        show_stderr = self.captured != "object" or env.get(
            "DEEPSH_SUBPROC_CAPTURED_PRINT_STDERR", True
        )
        if show_stderr:
    ...
    def stderr_prefix(self):
        ...
        if p is None:
            env = XSH.env
            t = env.get("DEEPSH_STDERR_PREFIX")
    ...
    def stderr_postfix(self):
        ...
        if p is None:
            env = XSH.env
            t = env.get("DEEPSH_STDERR_POSTFIX")
            s = xt.format_std_prepost(t, env=env)
            p = s.encode(
                encoding=env.get("DEEPSH_ENCODING"),
                errors=env.get("DEEPSH_ENCODING_ERRORS"),
