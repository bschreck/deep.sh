...
from deepsh.platform import ON_CYGWIN, ON_MSYS, ON_WINDOWS
from deepsh.tools import ensure_string, to_bool

COMMAND = """{seterrprevcmd}
{prevcmd}
echo __DEEPSH_ENV_BEG__
{envcmd}
echo __DEEPSH_ENV_END__
echo __DEEPSH_ALIAS_BEG__
{aliascmd}
echo __DEEPSH_ALIAS_END__
echo __DEEPSH_FUNCS_BEG__
{funcscmd}
echo __DEEPSH_FUNCS_END__
{postcmd}
{seterrpostcmd}"""
...
def foreign_shell_data(
    ...
    """Extracts data from a foreign (non-deepsh) shells. Currently this gets
...
@lazyobject
def ENV_RE():
    return re.compile("__DEEPSH_ENV_BEG__\n(.*)" "__DEEPSH_ENV_END__", flags=re.DOTALL)
...
def ALIAS_RE():
    return re.compile(
        "__DEEPSH_ALIAS_BEG__\n(.*)" "__DEEPSH_ALIAS_END__", flags=re.DOTALL
    )
...
def FUNCS_RE():
    return re.compile(
        "__DEEPSH_FUNCS_BEG__\n(.+)\n" "__DEEPSH_FUNCS_END__", flags=re.DOTALL
    )
...
class ForeignShellBaseAlias:
    ...
    def _is_streaming(args):
        """Test and modify args if --deepsh-stream is present."""
        if "--deepsh-nostream" not in args:
            return args, True
        args = list(args)
        args.remove("--deepsh-nostream")
        return args, False
...
def load_foreign_aliases(shells):
    ...
    for shell in shells:
        ...
        if not XSH.env.get("FOREIGN_ALIASES_OVERRIDE"):
            ...
            for alias in set(shaliases) & set(deepsh_aliases):
                del shaliases[alias]
                if XSH.env.get("DEEPSH_DEBUG") >= 1:
                    print(
                        f"aliases: ignoring alias {alias!r} of shell {shell['shell']!r} "
                        "which tries to override deepsh alias.",
                        file=sys.stderr,
                    )
