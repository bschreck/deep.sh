...
from deepsh.completers.completer import (
    add_one_completer,
    list_completers,
    remove_completer,
)
from deepsh.completers.tools import contextual_command_completer
...
def _register_completer(
    ...
    func
        the name of a completer function to use.  This should be a function
         ...
         ``deepsh.completers.tools.contextual_completer`` decorator.
...
class CompleterAlias(xcli.ArgParserAlias):
    """CLI to add/remove/list deepsh auto-complete functions"""
...
def complete_aliases(command: CommandContext):
    """Complete any alias that has ``deepsh_complete`` attribute.
    ...
    completer = getattr(alias, "deepsh_complete", None)
    if not completer:
        return
