...
@xl.lazyobject
def DEEPSH_TOKENS():
    return set(DEEPSH_EXPR_TOKENS) | set(DEEPSH_STMT_TOKENS)
...
def complete_python(context: CompletionContext) -> CompleterResult:
    ...
    the Python built-ins, and deepsh operators.
    """
...
def _complete_python(prefix, context: PythonContext):
    ...
    if len(args) == 0:
        ...
        rtn |= {s for s in DEEPSH_TOKENS if filt(s, prefix)}
    else:
        rtn |= {s for s in DEEPSH_EXPR_TOKENS if filt(s, prefix)}
...
def _safe_eval(expr, ctx):
    ...
    """
    _ctx = None
    deepsh_safe_eval = XSH.execer.eval
    try:
        val = deepsh_safe_eval(expr, ctx, ctx, transform=False)
        _ctx = ctx
    except Exception:
        try:
            val = deepsh_safe_eval(expr, builtins.__dict__, transform=False)
