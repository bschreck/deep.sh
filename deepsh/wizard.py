...
from deepsh.tools import backup_file, print_color, to_bool, to_bool_or_break
...
class PromptVisitor(StateVisitor):
    ...
    def visit_savejson(self, node):
        jstate = json.dumps(
            self.state, indent=1, sort_keys=True, default=serialize_deepsh_json
        )
        if node.check:
