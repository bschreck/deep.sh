...
from deepsh.color_tools import rgb_to_ints
from deepsh.prompt.base import PromptFormatter, default_prompt
...
def html_format(s, style="default"):
    buf = io.StringIO()
    proxy_style = deepsh_style_proxy(DeepshStyle(style))
...
def render_colors():
    ...
    for style in styles:
        try:
            ...
            logging.error(
                f"Failed to format Deepsh code {ex!r}. {style!r}", exc_info=True
            )
            display = source
