...
from deepsh.webconfig import tags as t
from deepsh.webconfig.file_writes import insert_into_deepshrc
from deepsh.webconfig.routes import Routes

RENDERERS: list[tp.Callable] = []


class DeepshConfigHTTPRequestHandler(server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        pprint(config)
        fname = insert_into_deepshrc(config)
        print("Wrote out to " + fname)
        self._send(b"received post request:<br>" + post_body)


def bind_server_to(
    port: int = 8421, handler_cls=DeepshConfigHTTPRequestHandler, browser=False
):
...
if __name__ == "__main__":
    # watchexec -r -e py -- python -m deepsh.webconfig --no-browser
    main()
