...
from . import deepsh_data
from .file_writes import insert_into_deepshrc
...
class Routes:
    ...
    def err(self, msg: str):
        html = deepsh_data.rst_to_html(msg)
...
class ColorsPage(Routes):
    ...
    def __init__(self, **kwargs):
        ...
        self.colors = dict(deepsh_data.render_colors())
        self.var_name = "DEEPSH_COLOR_STYLE"
...
class PromptsPage(Routes):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        prompts = deepsh_data.render_prompts(self.env)
...
class EnvVariablesPage(Routes):
    ...
    def get_rows(self):
        for name in sorted(self.env.keys()):
            ...
            html = deepsh_data.rst_to_html(envvar.doc)
            yield t.tr()[
