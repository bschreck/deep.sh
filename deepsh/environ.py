"""Environment for the deepsh shell."""
...
import deepsh.prompt.base as prompt
from deepsh import __version__ as DEEPSH_VERSION
from deepsh.ansi_colors import (
...
from deepsh.codecache import run_script_with_cache
from deepsh.dirstack import _get_cwd
from deepsh.events import events
from deepsh.lib.lazyasd import LazyBool, lazyobject
from deepsh.platform import (
...
events.doc(
    "on_pre_spec_run_ls",
    """
on_pre_spec_run_ls(spec: deepsh.built_ins.SubprocSpec) -> None
...
class LsColors(cabc.MutableMapping):
    """Helps convert to/from $LS_COLORS format, respecting the deepsh color style.
    ...
    @property
    def style_name(self):
        """Current DEEPSH_COLOR_STYLE value"""
        env = getattr(XSH, "env", {}) or {}
        env_style_name = env.get("DEEPSH_COLOR_STYLE", "default")
    ...
    @property
    def style(self):
        """The ANSI color style for the current DEEPSH_COLOR_STYLE"""
    ...
    def fromstring(cls, s):
        ...
        # deepsh names should be done from defaults
        ...
        for item in s.split(":"):
            ...
            if not eq:
                ...
                try:
                    ...
                    print("deepsh:warning:" + str(e), file=sys.stderr)
                    ini_dict[key] = ("RESET",)
        return cls(ini_dict)
...
def default_value(f):
    ...
    f._deepsh_callable_default = True
    return f


def is_callable_default(x):
    """Checks if a value is a callable default."""
    return callable(x) and getattr(x, "_deepsh_callable_default", False)


DEFAULT_TITLE = "{current_job:{} | }{user}@{hostname}: {cwd} | deepsh"


@default_value
def deepsh_data_dir(env):
    """Ensures and returns the $DEEPSH_DATA_DIR"""
    xdd = os.path.expanduser(os.path.join(env.get("XDG_DATA_HOME"), "deepsh"))
    os.makedirs(xdd, exist_ok=True)
    return xdd


@default_value
def deepsh_cache_dir(env):
    """Ensures and returns the $DEEPSH_CACHE_DIR"""
    xdd = os.path.expanduser(os.path.join(env.get("XDG_CACHE_HOME"), "deepsh"))
    os.makedirs(xdd, exist_ok=True)
    return xdd


@default_value
def deepsh_config_dir(env):
    """``$XDG_CONFIG_HOME/deepsh``"""
    xcd = os.path.expanduser(os.path.join(env.get("XDG_CONFIG_HOME"), "deepsh"))
...
@default_value
def deepsh_sys_config_dir(env):
    """
    On Linux & Mac OSX: ``'/etc/deepsh'``
    On Windows: ``'%ALLUSERSPROFILE%\\\\deepsh'``
    """
    if ON_WINDOWS:
        etc_path = os_environ["ALLUSERSPROFILE"]
    else:
        etc_path = "/etc"
    return os.path.join(etc_path, "deepsh")


def deepshconfig(env):
    """Ensures and returns the $DEEPSHCONFIG"""
    xcd = env.get("DEEPSH_CONFIG_DIR")
    xc = os.path.join(xcd, "config.json")
    return xc


def get_home_deepshrc_path():
    """Cross-platform implementation of getting ``~/.deepshrc`` path."""
    return str((Path("~") / ".deepshrc").expanduser())


@default_value
def default_deepshrc(env) -> "tuple[str, ...]":
    """
    ``['$DEEPSH_SYS_CONFIG_DIR/deepshrc', '$DEEPSH_CONFIG_DIR/deepsh/rc.xsh', '~/.deepshrc']``
    """

    dxrc = (
        os.path.join(deepsh_sys_config_dir(env), "deepshrc"),
        os.path.join(deepsh_config_dir(env), "rc.xsh"),
        get_home_deepshrc_path(),
    )
    # Check if old config file exists and issue warning
    old_config_filename = deepshconfig(env)
    if os.path.isfile(old_config_filename):
        print(
            "WARNING! old style configuration ("
            + old_config_filename
            + ") is no longer supported. "
            + "Please migrate to deepshrc."
        )
    return dxrc
...
def get_config_paths(env: "Env", name: str):
    return (
        os.path.join(deepsh_sys_config_dir(env), name),
        os.path.join(deepsh_config_dir(env), name),
    )


@default_value
def default_deepshrcdir(env):
    """``['$DEEPSH_SYS_CONFIG_DIR/rc.d', '$DEEPSH_CONFIG_DIR/rc.d']``\n"""
...
def default_completer_dirs(env):
    ...
    1. ``$DEEPSH_CONFIG_DIR/completions`` - user level completions
    2. ``$DEEPSH_SYS_CONFIG_DIR/completions`` - system level completions
    ...
    return [
        os.path.join(env["DEEPSH_CONFIG_DIR"], "completions"),
        os.path.join(env["DEEPSH_SYS_CONFIG_DIR"], "completions"),
        os.path.join(env["DEEPSH_DATA_DIR"], "generated_completions"),
    ] + [
        os.path.join(parent, "deepsh", "vendor_completions")
        for parent in env["XDG_DATA_DIRS"]
    ]


@default_value
def deepsh_append_newline(env):
    """Appends a newline if we are in interactive mode"""
    return env.get("DEEPSH_INTERACTIVE", False)
...
class GeneralSetting(Xettings):
    ...
    HOSTTYPE = Var.with_default(
        ...
        doc="Automatically set to a string that fully describes the system type on which deepsh is executing.",
        type_str="str",
    )
    ...
    TERM = Var.no_default(
        ...
        "options:\n\n"
        "* Set this from the program that launches deepsh. On POSIX systems, \n"
        "  this can be performed by using env, e.g. \n"
        "  ``/usr/bin/env TERM=xterm-color deepsh`` or similar.\n"
        "* From the deepsh command line, namely ``deepsh -DTERM=xterm-color``.\n"
        '* In the config file with ``{"env": {"TERM": "xterm-color"}}``.\n'
        "* Lastly, in deepshrc with ``$TERM``\n\n"
    ...
    UPDATE_OS_ENVIRON = Var.with_default(
        ...
        "when the deepsh environment changes. The environment can be reset to "
        "the default value by calling ``__deepsh__.env.undo_replace_env()``",
    )
    ...
    DEEPSH_CONFIG_DIR = Var.with_default(
        deepsh_config_dir,
        "This is the location where deepsh user-level configuration information is stored.",
        is_configurable=False,
        type_str="str",
    )
    DEEPSH_SYS_CONFIG_DIR = Var.with_default(
        deepsh_sys_config_dir,
        "This is the location where deepsh system-level configuration information is stored.",
        is_configurable=False,
        type_str="str",
    )
    ...
    DEEPSH_DEBUG = Var(
        ...
        bool_or_int_to_str,
        0,
        "Sets the deepsh debugging level. This may be an integer or a boolean. "
    ...
    DEEPSH_DATA_DIR = Var.with_default(
        deepsh_data_dir,
        "This is the location where deepsh data files are stored, such as history, generated completers ...",
        doc_default="``$XDG_DATA_HOME/deepsh``",
        type_str="str",
    )

    DEEPSH_ENCODING = Var.with_default(
        DEFAULT_ENCODING,
        "This is the encoding that deepsh should use for subprocess operations.",
        doc_default="``sys.getdefaultencoding()``",
        type_str="str",
    )
    DEEPSH_ENCODING_ERRORS = Var.with_default(
        "surrogateescape",
    ...
    DEEPSH_INTERACTIVE = Var.with_default(
        True,
        "``True`` if deepsh is running interactively, and ``False`` otherwise. "
        "It's highly recommended to use this variable in your ``deepshrc`` files "
        "to split the code execution for interactive and non-interactive modes.",
        is_configurable=False,
    )
    DEEPSH_LOGIN = Var.with_default(
        False,
        "``True`` if deepsh is running as a login shell, and ``False`` otherwise.",
        is_configurable=False,
    )

    DEEPSH_MODE = Var.with_default(
        default="interactive",  # In sync with ``main.py``.
        doc="A string value representing the current deepsh execution mode: "
        ...
        "you plan to ``source``, use ``$DEEPSH_INTERACTIVE`` as the flag instead.",
        type_str="str",
    )

    DEEPSH_SOURCE = Var.with_default(
        "",
        "When running a deepsh script, this variable contains the absolute path "
        "to the currently executing script's file.",
        is_configurable=False,
    )
    DEEPSH_STORE_STDIN = Var.with_default(
        False,
        "Whether or not to store the stdin that is supplied to the "
        "``!()`` and ``![]`` operators.",
    )
    DEEPSH_STYLE_OVERRIDES = Var(
        is_tok_color_dict,
        to_tok_color_dict,
        dict_to_str,
        {},
        ...
        "    - ``pygments.token.Token`` - ``$DEEPSH_STYLE_OVERRIDES[Token.Keyword] = '#ff0000'``\n"
        "    - pygments token name (string) - ``$DEEPSH_STYLE_OVERRIDES['Token.Keyword'] = '#ff0000'``\n"
        "    - ptk style name (string) - ``$DEEPSH_STYLE_OVERRIDES['pygments.keyword'] = '#ff0000'``\n\n"
...
class SubprocessSetting(Xettings):
    ...
    RAISE_SUBPROC_ERROR = Var.with_default(
        ...
        "This is most useful in deepsh scripts or modules where failures "
    ...
    DEEPSH_SUBPROC_CAPTURED_PRINT_STDERR = Var.with_default(
        False,
        "If ``True`` the stderr from captured subproc will be printed automatically.",
    )
    DEEPSH_SUBPROC_OUTPUT_FORMAT = Var.with_default(
        "stream_lines",
    ...
    DEEPSH_CAPTURE_ALWAYS = Var.with_default(
        False,
        "Try to capture output of commands run without explicit capturing.\n"
        "If True, deepsh will capture the output of commands run directly or in ``![]``"
        ...
        "  For more information see discussion at https://github.com/deepsh/deepsh/issues/3672.\n"
    ...
    THREAD_SUBPROCS = Var(
        ...
        bool_or_none_to_str,
        not ON_CYGWIN,
        "Note: The ``$DEEPSH_CAPTURE_ALWAYS`` variable introduces finer control "
        ...
        "If True, deepsh is able capture & store the stdin, stdout, and stderr"
        " of threadable subprocesses.\n"
        "The disadvantages are listed in ``$DEEPSH_CAPTURE_ALWAYS``.\n"
        ...
        "threading when loading deepshrc files. This is done because Bash "
        "was automatically placing new deepsh instances in the background "
        "at startup when threadable subprocs were used. Please see "
        "https://github.com/deepsh/deepsh/pull/3705 for more information.\n",
    )
    DEEPSH_APPEND_NEWLINE = Var.with_default(
        deepsh_append_newline,
        "Append new line when a partial line is preserved in output.",
        doc_default="``$DEEPSH_INTERACTIVE``",
        type_str="bool",
    )
    DEEPSH_PROC_FREQUENCY = Var.with_default(
        1e-4,
        "The process frequency is the time that "
        "deepsh process threads sleep for while running command pipelines. "
        "The value has units of seconds [s].",
    )
    DEEPSH_TRACE_SUBPROC = Var(
        default=False,
    ...
    DEEPSH_TRACE_SUBPROC_FUNC = Var.with_default(
        None,
        doc=(
            "A callback function used to format the trace output shown when $DEEPSH_TRACE_SUBPROC=True."
        ),
        doc_default="""\
...
class ErrorHandlingSetting(Xettings):
    ...
    DEEPSH_SHOW_TRACEBACK = Var.with_default(
        False,
    ...
    DEEPSH_TRACEBACK_LOGFILE = Var(
        is_logfile_opt,
        to_logfile_opt,
        logfile_opt_to_str,
        None,
        "Specifies a file to store the traceback log to, regardless of whether "
        "``DEEPSH_SHOW_TRACEBACK`` has been set. Its value must be a writable file "
...
class LangSetting(Xettings):
    ...
    DEEPSH_DATETIME_FORMAT = Var.with_default(
        "%Y-%m-%d %H:%M",
...
class CacheSetting(Xettings):
    ...
    DEEPSH_CACHE_SCRIPTS = Var.with_default(
        True,
        "Controls whether the code for scripts run from deepsh will be cached"
        " (``True``) or re-compiled each time (``False``).",
    )

    DEEPSH_CACHE_EVERYTHING = Var.with_default(
        False,
        "Controls whether all code (including code entered at the interactive"
        " prompt) will be cached.",
    )

    DEEPSH_CACHE_DIR = Var.with_default(
        deepsh_cache_dir,
        "This is the location where cache files used by deepsh are stored, such as commands-cache...",
        doc_default="``$XDG_CACHE_HOME/deepsh``",
        type_str="str",
    )
...
class ChangeDirSetting(Xettings):
    ...
    CDPATH = Var.with_default(
        ...
        "with Bash, deepsh always prefer an existing relative path.",
        type_str="env_path",
    )
...
class InterpreterSetting(Xettings):
    ...
    FOREIGN_ALIASES_SUPPRESS_SKIP_MESSAGE = Var.with_default(
        ...
        "because it already exists in deepsh.",
        is_configurable=True,
    )
    FOREIGN_ALIASES_OVERRIDE = Var.with_default(
        False,
        "Whether or not foreign aliases should override deepsh aliases "
        "with the same name. Note that setting of this must happen in the "
        "environment that deepsh was started from. "
        "It cannot be set in the ``.deepshrc`` as loading of foreign aliases happens before"
        "``.deepshrc`` is parsed",
        is_configurable=True,
    )
...
class PromptSetting(Xettings):
    ...
    PROMPT_TOKENS_FORMATTER = Var(
        ...
        doc_default="``deepsh.prompt.base.prompt_tokens_formatter_default``",
    )
    RIGHT_PROMPT = Var(
        is_string_or_callable,
        ensure_string,
    ...
    SHELL_TYPE = Var.with_default(
        ...
        "It also accepts a class type that inherits from ``deepsh.shells.base_shell.BaseShell``",
        doc_default="``best``",
    )
    SUGGEST_COMMANDS = Var.with_default(
        True,
        "When a user types an invalid command, deepsh will try to offer "
        "suggestions of similar valid commands if this is True.",
    )
    SUGGEST_MAX_NUM = Var.with_default(
        5,
        "deepsh will show at most this many suggestions in response to an "
    ...
    DEEPSH_HISTORY_MATCH_ANYWHERE = Var.with_default(
        False,
    ...
    DEEPSH_STDERR_PREFIX = Var.with_default(
        "",
        ...
        "conjunction with ``$DEEPSH_STDERR_POSTFIX`` to close out the block."
        "For example, to have stderr appear on a red background, the "
        'prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}".',
    )
    DEEPSH_STDERR_POSTFIX = Var.with_default(
        "",
        "A format string, using the same keys and colors as ``$PROMPT``, that "
        "is appended whenever stderr is displayed. This may be used in "
        "conjunction with ``$DEEPSH_STDERR_PREFIX`` to start the block."
        "For example, to have stderr appear on a red background, the "
        'prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}".',
    )
    DEEPSH_SUPPRESS_WELCOME = Var.with_default(
        False,
...
class PromptHistorySetting(Xettings):
    ...
    DEEPSH_HISTORY_BACKEND = Var(
        ...
        "``DEEPSH_HISTORY_BACKEND`` also accepts a class type that inherits "
        "from ``deepsh.history.base.History``, or its instance.",
    )
    DEEPSH_HISTORY_FILE = Var.with_default(
        None,
    ...
    DEEPSH_STORE_STDOUT = Var.with_default(
        False,
        "Store the ``stdout`` and ``stderr`` streams to the history. "
        "Requires that DEEPSH_CAPTURE_ALWAYS is True.",
    )
    DEEPSH_HISTORY_SAVE_CWD = Var.with_default(
        True,
        "Save current working directory to the history.",
        doc_default="True",
    )
    DEEPSH_HISTORY_IGNORE_REGEX = Var(
        is_regex,
        to_itself,
        ensure_string,
        None,
        ...
        "Example: ``$DEEPSH_HISTORY_IGNORE_REGEX = '(^echo|^.*\\#\\#\\#\\s*|.*\\#\\#\\#\\s*$)'``"
        " - skip commands that start from ``echo`` or ``###``, or end from ``###``.",
    )
    DEEPSH_HISTORY_SIGINT_FLUSH = Var.with_default(
        True,
...
class PTKSetting(PromptSetting):  # sub-classing -> sub-group
    ...
    DEEPSH_AUTOPAIR = Var.with_default(
        False,
        "Whether Deepsh will auto-insert matching parentheses, brackets, and "
        "quotes. Only available under the prompt-toolkit shell.",
    )
    DEEPSH_COPY_ON_DELETE = Var.with_default(
        False,
    ...
    DEEPSH_USE_SYSTEM_CLIPBOARD = Var.with_default(
        True,
    ...
    DEEPSH_CTRL_BKSP_DELETION = Var.with_default(
        False,
...
class AutoCompletionSetting(Xettings):
    ...
    ALIAS_COMPLETIONS_OPTIONS_BY_DEFAULT = Var.with_default(
        doc="""\
If True, :py:class:`deepsh.completers.argparser.ArgparseCompleter` based completions
...
the :py:class:`deepsh.completers.argparser.ArgparseCompleter` based completions.
...
This is to reduce the noise in generated completions.""",
    ...
    DEEPSH_BASH_PATH_OVERRIDE = Var.with_default(
        None,
        ...
        "``deepsh.platform.bash_command``.",
        doc_default="None",
    )
    DEEPSH_COMPLETER_DIRS = Var.with_default(
        default_completer_dirs,
        """\
A list of paths where Deepsh searches for command completions.
...
``deepsh_complete(ctx: CommandContext) -> Iterator[RichCompletion|str]``.
""",
        type_str="env_path",
    )
    DEEPSH_TRACE_COMPLETIONS = Var.with_default(
        False,
...
class Env(cabc.MutableMapping):
    """A deepsh environment, whose variables have limited typing
    ...
    * DEEPSH_HISTORY_SIZE: this variable is an (int | float, str) tuple.
    * LC_* (locale categories): locale category names get/set the Python
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "PATH" not in self._d:
            ...
            # it can be modified in-place in the deepshrc file
...
def locate_binary(name):
    ...
    with scanning all files in ``$PATH``. First of all take a look into ``deepsh.specs.executables``
...
def deepshrc_context(
    rcfiles=None, rcdirs=None, execer=None, ctx=None, env=None, login=True
):
    """
    Attempts to read in all deepshrc files (and search deepshrc directories),
    ...
    if rcfiles is not None:
        for rcfile in rcfiles:
            if os.path.isfile(rcfile):
                status = deepsh_script_run_control(
...
def windows_foreign_env_fixes(ctx):
    ...
    # which messes up these environment variables for deepsh.
...
def deepsh_script_run_control(filename, ctx, env, execer=None, login=True):
    """Loads a deepsh file and applies it as a run control.
    ...
    with swap_values(ctx, updates):
        ...
        if exc_info != (None, None, None):
            ...
            if err_type is SyntaxError:
                msg = "syntax error in deepsh run control file {0!r}: {1!s}"
            else:
                msg = "error running deepsh run control file {0!r}: {1!s}"
...
def default_env(env=None):
    ...
    # other shells' PROMPT definitions generally don't work in DEEPSH:
    try:
        del ctx["PROMPT"]
    except KeyError:
        pass
