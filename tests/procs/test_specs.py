...
from deepsh.procs.proxies import STDOUT_DISPATCHER, ProcProxy, ProcProxyThread
from deepsh.procs.specs import (
    DecoratorAlias,
...
from deepsh.pytest.tools import ON_WINDOWS, VER_MAJOR_MINOR, skip_if_on_windows
from deepsh.tools import DeepshError
...
def test_cmds_to_specs_thread_subproc(xession):
    ...
    # DEEPSH_CAPTURE_ALWAYS=False should disable interactive threaded subprocs
    env["DEEPSH_CAPTURE_ALWAYS"] = False
    ...
    env["DEEPSH_CAPTURE_ALWAYS"] = True
...
def test_cmds_to_specs_capture_stdout_not_stderr(thread_subprocs, deepsh_session):
    env = deepsh_session.env
    cmds = (["ls", "/root"],)
...
def test_capture_always(
    capfd, thread_subprocs, capture_always, alias_type, pipe, monkeypatch, deepsh_session
):
    if not thread_subprocs and alias_type in ["func", "exec"]:
        if pipe:
            return pytest.skip("https://github.com/deepsh/deepsh/issues/4443")
        else:
            return pytest.skip("https://github.com/deepsh/deepsh/issues/4444")
    ...
    if alias_type:
        ...
        if alias_type == "func":
            deepsh_session.aliases["tst"] = (
                lambda: run_subproc([first_cmd], "hiddenobject") and None
            )  # Don't return a value
        elif alias_type == "exec":
            first_cmd = " ".join(repr(arg) for arg in first_cmd)
            deepsh_session.aliases["tst"] = f"![{first_cmd}]"
        else:
            # alias_type == "simple"
            deepsh_session.aliases["tst"] = first_cmd

        cmds[0] = ["tst"]

    env["THREAD_SUBPROCS"] = thread_subprocs
    env["DEEPSH_CAPTURE_ALWAYS"] = capture_always
...
@skip_if_on_windows
@pytest.mark.flaky(reruns=3, reruns_delay=2)
def test_callias_captured_redirect(deepsh_session, tmpdir):
    @deepsh_session.aliases.register("a")
    def _a(a, i, o, e):
        print("print_stdout")
        deepsh_session.subproc_captured_stdout(["echo", "cap_stdout"])
        deepsh_session.subproc_captured_object(["echo", "cap_object"])
        deepsh_session.subproc_captured_hiddenobject(["echo", "hiddenobject"])
        deepsh_session.subproc_uncaptured(["echo", "uncaptured"])
...
@pytest.mark.flaky(reruns=3, reruns_delay=2)
def test_interrupted_process_returncode(deepsh_session, captured, interactive):
    deepsh_session.env["DEEPSH_INTERACTIVE"] = interactive
...
def test_proc_raise_subproc_error(deepsh_session):
    deepsh_session.env["RAISE_SUBPROC_ERROR"] = False
    ...
    deepsh_session.env["RAISE_SUBPROC_ERROR"] = True
    specs = cmds_to_specs(cmd := [["ls", "nofile"]], captured="stdout")
    exception = None
    try:
        (p := _run_command_pipeline(specs, cmd)).end()
...
def test_specs_with_suspended_captured_process_pipeline(
    deepsh_session, suspended_pipeline
):
    deepsh_session.env["DEEPSH_INTERACTIVE"] = True
...
@pytest.mark.flaky(reruns=3, reruns_delay=1)
def test_subproc_output_format(cmds, exp_stream_lines, exp_list_lines, deepsh_session):
    deepsh_session.env["DEEPSH_SUBPROC_OUTPUT_FORMAT"] = "stream_lines"
    output = run_subproc(cmds, "stdout")
    assert output == exp_stream_lines

    deepsh_session.env["DEEPSH_SUBPROC_OUTPUT_FORMAT"] = "list_lines"
    output = run_subproc(cmds, "stdout")
...
def test_on_command_not_found_fires(xession):
    ...
    def my_handler(cmd, **kwargs):
        nonlocal fired
        assert cmd[0] == "deepshcommandnotfound"
        fired = True

    xession.builtins.events.on_command_not_found(my_handler)
    subproc = SubprocSpec.build(["deepshcommandnotfound"])
    with pytest.raises(DeepshError) as expected:
        subproc.run()
    assert "command not found: 'deepshcommandnotfound'" in str(expected.value)
    assert fired
