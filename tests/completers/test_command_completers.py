...
from deepsh.completers.commands import complete_command, complete_skipper
from deepsh.parsers.completion_context import (
    CommandArg,
    CommandContext,
    CompletionContext,
)
from deepsh.pytest.tools import ON_WINDOWS, completions_from_result, skip_if_on_windows
...
def test_argparse_completer(check_completer, monkeypatch):
    assert check_completer("deepsh", prefix="-").issuperset(
        {
            "--cache-everything",
            "--help",
            "--interactive",
...
def test_argparse_completer_after_option(check_completer, tmp_path):
    ...
    assert check_completer("deepsh --no-rc", prefix)
